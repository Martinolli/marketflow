...
logger = get_logger("FairPriceCalculator")
...
def main():
    ...
    """
    Main function to execute the fair price calculation.
    Args:
        None
    Returns:
        None
    """
    parser = argparse.ArgumentParser(description="Calculate fair price of a stock.")
    parser.add_argument("ticker", nargs= '+', type=str, help="Stock ticker symbol")
    args = parser.parse_args()

    for tk in args.ticker:
        try:
            ttm = fetch_financials_ttm(tk)
            logger.info(f"Fetched TTM financials for {tk}: {ttm}")

            if ttm["fcf"] < 0:
                print(f"\n[{tk}] Running Venture-DCF (negative FCF ramp)...")
                ev, fcf_path = venture_dcf(
                    current_fcf=ttm["fcf"],
                    target_fcf=300e6,
                    years_to_target=6,
                    post_target_growth=0.05,
                    discount_rate=0.12,
            ...
            print("\n" + "="*40)
            print(f"[{tk}] Estimated Fair Price: ${fair_price:,.2f}")
            print("="*40)
...
def _pick(section, *keys, default=0.0):
    ...
    Returns the first non-zero value found, or the default.
    """
    for key in keys:
        try:
...
def fetch_financials_ttm(ticker: str):
    ...
    for i in range(4):
        ...
        # FIXED: Handle missing depreciation data - try multiple sources
        depreciation = 0.0
        # Try cash flow statement first
        depreciation = _get_val(cf_curr, "depreciation_and_amortization")
        if depreciation == 0.0:
            # Try income statement as backup
            is_curr = current_report.financials.income_statement
            depreciation = _get_val(is_curr, "depreciation_and_amortization")
        
        if depreciation == 0.0:
            print(f"  - Warning: Depreciation not found for quarter ending {current_report.end_date}, using 0.")
    ...
    # FIXED: Handle missing short-term debt - use current_liabilities as proxy or default to 0
    short_term_debt, short_debt_key = _pick(latest_bs, "current_debt", "short_term_debt")
    if not short_debt_key:
        # If no specific short-term debt, we could use a portion of current liabilities
        # or just default to 0 for conservative estimate
        print("  - Warning: Short-term debt not found, using 0.")
        short_term_debt = 0.0
    ...
    # If still no cash found, try using net_cash_flow from cash flow statement as proxy
    if not cash_key or latest_cash == 0.0:
        print("  - Warning: Cash not found in balance sheet, trying cash flow statement...")
        latest_cash, _ = _pick(latest_cfs, "net_cash_flow")
        if latest_cash == 0.0:
            print("  - Warning: No cash data found, using 0.")
