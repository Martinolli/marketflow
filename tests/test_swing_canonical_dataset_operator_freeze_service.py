...
from marketflow.services import swing_canonical_dataset_operator_freeze_service as freeze
...
def _attestation(**overrides) -> dict:
    values = {
        ...
        "operator_attestation_phrase": freeze.REQUIRED_SWING_CANONICAL_DATASET_OPERATOR_ATTESTATION_PHRASE,
        "operator_confirms_swing_review_package_digest": freeze.EXPECTED_SWING_REVIEW_PACKAGE_DIGEST,
        "operator_confirms_swing_candidate_digest": freeze.EXPECTED_SWING_CANDIDATE_DIGEST,
    ...
    return freeze.build_swing_canonical_dataset_operator_attestation_v1(**values)


def _frozen(**attestation_overrides) -> dict:
    return freeze.build_swing_canonical_dataset_frozen_v1(operator_attestation=_attestation(**attestation_overrides))
...
def _recompute(artifact: dict) -> None:
    ...
    artifact["swing_canonical_dataset_frozen_semantic_digest"] = freeze.swing_canonical_dataset_frozen_semantic_digest_v1(
        artifact
    )
...
def test_operator_attestation_builder_creates_required_fields():
    ...
    assert attestation["operator_decision"] == "APPROVE_SWING_CANONICAL_DATASET_FREEZE"
    assert attestation["operator_reference"] == "TEST_OPERATOR"
    assert attestation["operator_attestation_phrase"] == freeze.REQUIRED_SWING_CANONICAL_DATASET_OPERATOR_ATTESTATION_PHRASE
...
def test_frozen_artifact_builds_offline_without_provider_calls(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(freeze.swing.acquisition, "fetch_massive_custom_bars_v1", fail_provider_call)
...
def test_artifact_kind_is_swing_canonical_dataset_frozen():
    assert _frozen()["artifact_kind"] == "SWING_CANONICAL_DATASET_FROZEN"


def test_freeze_status_is_swing_canonical_dataset_frozen():
    assert _frozen()["freeze_status"] == "SWING_CANONICAL_DATASET_FROZEN"


def test_swing_canonical_dataset_frozen_is_true():
    assert _frozen()["swing_canonical_dataset_frozen"] is True


def test_canonical_eligibility_remains_false():
    assert _frozen()["canonical_eligibility"] is False
...
def test_swing_review_package_digest_matches_expected():
    assert _frozen()["source_swing_review_package_semantic_digest"] == freeze.EXPECTED_SWING_REVIEW_PACKAGE_DIGEST


def test_swing_candidate_digest_matches_expected():
    assert _frozen()["source_swing_candidate_digest"] == freeze.EXPECTED_SWING_CANDIDATE_DIGEST
...
def test_swing_bar_count_is_1988():
    assert _frozen()["swing_bar_count"] == 1988


def test_2025_01_cross_check_passed_and_has_40_bars():
    artifact = _frozen()

    assert artifact["cross_check_2025_01_status"] == "PASSED"
    assert artifact["cross_check_2025_01_swing_bars"] == 40
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("operator_confirms_swing_review_package_digest", "0" * 64, "operator_review_digest_confirmation_matches"),
        ("operator_confirms_swing_candidate_digest", "0" * 64, "operator_candidate_digest_confirmation_matches"),
        ...
        ("operator_confirms_swing_bar_count", 1987, "operator_confirms_swing_bar_count"),
...
def test_wrong_operator_attestation_values_are_rejected(field: str, value, match: str):
    with pytest.raises(freeze.SwingCanonicalDatasetOperatorFreezeError, match=match):
        _frozen(**{field: value})


def test_missing_attestation_is_rejected():
    with pytest.raises(freeze.SwingCanonicalDatasetOperatorFreezeError, match="operator_attestation"):
        freeze.build_swing_canonical_dataset_frozen_v1(operator_attestation=None)  # type: ignore[arg-type]


def test_review_package_blocker_count_is_rejected():
    package = freeze.review.build_swing_canonical_dataset_candidate_review_package_v1()
    ...
    package["swing_canonical_dataset_review_package_semantic_digest"] = (
        freeze.review.swing_canonical_dataset_review_package_semantic_digest_v1(package)
    )

    with pytest.raises(freeze.SwingCanonicalDatasetOperatorFreezeError, match="source SWING review package invalid"):
        freeze.build_swing_canonical_dataset_frozen_v1(swing_review_package=package, operator_attestation=_attestation())
...
def test_frozen_artifact_digest_is_deterministic_across_repeated_builds():
    ...
    assert first["swing_canonical_dataset_frozen_semantic_digest"] == second["swing_canonical_dataset_frozen_semantic_digest"]
    assert first["swing_canonical_dataset_frozen_semantic_digest"] == freeze.swing_canonical_dataset_frozen_semantic_digest_v1(first)


@pytest.mark.parametrize(
    ("path", "value", "match"),
    [
        (("source_swing_review_package_semantic_digest",), "0" * 64, "source_swing_review_package_semantic_digest"),
        (("source_swing_review_status",), "NOT_READY", "source_swing_review_status"),
        (("source_swing_review_blocker_count",), 1, "source_swing_review_blocker_count"),
        (("source_swing_candidate_digest",), "0" * 64, "source_swing_candidate_digest"),
        ...
        (("swing_canonical_dataset_frozen",), False, "swing_canonical_dataset_frozen"),
...
def test_validator_rejects_invalid_frozen_artifact_mutations(path: tuple[str, ...], value, match: str):
    ...
    with pytest.raises(freeze.SwingCanonicalDatasetOperatorFreezeError, match=match):
        freeze.validate_swing_canonical_dataset_frozen_v1(artifact)
...
def test_remaining_roadmap_includes_required_future_work():
    ...
    assert "POSITION_SWING canonical dataset operator review." in roadmap
    assert "POSITION_SWING canonical dataset freeze." in roadmap
...
def test_freeze_checklist_summary_counts_and_blockers():
    ...
    assert summary["swing_canonical_dataset_freeze_authorized_by_operator"] is True
...
def test_markdown_writer_includes_required_sections_and_guardrails():
    markdown = freeze.build_swing_canonical_dataset_frozen_markdown_v1(_frozen())
...
def test_write_frozen_artifact_writes_json_without_overwrite(tmp_path: Path):
    result = freeze.write_swing_canonical_dataset_frozen_v1(tmp_path, operator_attestation=_attestation())

    assert result["artifact_kind"] == "SWING_CANONICAL_DATASET_FROZEN"
    assert result["frozen_payload_digest"]
    with pytest.raises(freeze.SwingCanonicalDatasetOperatorFreezeError, match="already exists"):
        freeze.write_swing_canonical_dataset_frozen_v1(tmp_path, operator_attestation=_attestation())


def test_freeze_service_exports_are_public():
    import marketflow.services as services

    assert services.ARTIFACT_KIND_SWING_CANONICAL_DATASET_FROZEN == "SWING_CANONICAL_DATASET_FROZEN"
    assert services.SWING_CANONICAL_DATASET_FROZEN == "SWING_CANONICAL_DATASET_FROZEN"
    assert (
        services.REQUIRED_SWING_CANONICAL_DATASET_OPERATOR_ATTESTATION_PHRASE
        == freeze.REQUIRED_SWING_CANONICAL_DATASET_OPERATOR_ATTESTATION_PHRASE
    )
    assert services.build_swing_canonical_dataset_operator_attestation_v1 is freeze.build_swing_canonical_dataset_operator_attestation_v1
    assert services.build_swing_canonical_dataset_frozen_v1 is freeze.build_swing_canonical_dataset_frozen_v1
    assert services.validate_swing_canonical_dataset_frozen_v1 is freeze.validate_swing_canonical_dataset_frozen_v1
    assert services.write_swing_canonical_dataset_frozen_v1 is freeze.write_swing_canonical_dataset_frozen_v1
    assert services.build_swing_canonical_dataset_frozen_markdown_v1 is freeze.build_swing_canonical_dataset_frozen_markdown_v1
