...
from marketflow.services import (
    dividend_event_authority_candidate_operator_review_service as review,
)
...
def _package() -> dict[str, Any]:
    return review.build_dividend_event_authority_candidate_review_package_v1()
...
def test_review_package_builds_offline_without_provider_calls(
    ...
    monkeypatch.setattr(
        review.candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.review_service.candidate_service.plan_review.plan.results_review.provider,
...
def test_artifact_kind_status_schema_and_binding_mode_are_exact():
    ...
    assert package["artifact_kind"] == (
        review.ARTIFACT_KIND_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE
    )
    assert package["review_status"] == (
        review.DIVIDEND_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_READY
    )
    assert package["schema_version"] == (
        review.SCHEMA_VERSION_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_REVIEW_V1
    )
    assert package["dividend_event_authority_candidate_binding_mode"] == (
        review.DIVIDEND_EVENT_AUTHORITY_CANDIDATE_STATUS_BINDING
    )


def test_supplied_candidate_uses_object_binding_mode():
    candidate = review.candidate_service.build_dividend_event_authority_candidate_v1()

    package = review.build_dividend_event_authority_candidate_review_package_v1(
        candidate=candidate
    )

    assert package["dividend_event_authority_candidate_binding_mode"] == (
        review.DIVIDEND_EVENT_AUTHORITY_CANDIDATE_OBJECT_BINDING
    )


def test_reviewed_dividend_event_candidate_evidence_is_bound():
    package = _package()

    assert package["reviewed_dividend_event_authority_candidate_kind"] == (
        review.candidate_service.ARTIFACT_KIND_DIVIDEND_EVENT_AUTHORITY_CANDIDATE
    )
    assert package["reviewed_dividend_event_authority_candidate_status"] == (
        review.candidate_service.DIVIDEND_EVENT_AUTHORITY_READY_FOR_OPERATOR_REVIEW
    )
    assert package["reviewed_dividend_event_authority_candidate_digest"] == (
        EXPECTED_CANDIDATE_DIGEST
    )
    assert package["dividend_event_authority_candidate_digest"] == EXPECTED_CANDIDATE_DIGEST
    assert package["reviewed_dividend_event_authority_candidate_checklist_total"] == 96
    assert package["reviewed_dividend_event_authority_candidate_checklist_passed"] == 96
    assert package["reviewed_dividend_event_authority_candidate_checklist_failed"] == 0
    assert package["reviewed_dividend_event_authority_candidate_blocker_count"] == 0
...
def test_source_digest_chain_is_bound():
    ...
    assert package["post_identity_freeze_registry_inventory_approval_digest"] == (
        review.candidate_service.split_review.candidate_service.approval.review.plan.EXPECTED_POST_IDENTITY_FREEZE_REGISTRY_INVENTORY_APPROVAL_DIGEST
    )
    assert package["post_identity_freeze_registry_inventory_candidate_review_package_digest"] == (
        review.candidate_service.split_review.candidate_service.approval.review.plan.approval_service.EXPECTED_POST_IDENTITY_FREEZE_REGISTRY_INVENTORY_CANDIDATE_REVIEW_PACKAGE_DIGEST
    )
    assert package["post_identity_freeze_registry_inventory_candidate_digest"] == (
        review.candidate_service.split_review.candidate_service.approval.review.plan.approval_service.EXPECTED_POST_IDENTITY_FREEZE_REGISTRY_INVENTORY_CANDIDATE_DIGEST
    )
    assert package["identity_authority_freeze_digest"] == (
        review.candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.EXPECTED_IDENTITY_AUTHORITY_FREEZE_DIGEST
    )
    assert package["identity_authority_candidate_review_package_digest"] == (
        review.candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_IDENTITY_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_DIGEST
    )
    assert package["identity_authority_candidate_digest"] == (
        review.candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_IDENTITY_AUTHORITY_CANDIDATE_DIGEST
    )
    assert package["live_ticker_validation_results_review_package_digest"] == (
        review.candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_LIVE_TICKER_VALIDATION_RESULTS_REVIEW_PACKAGE_DIGEST
    )
    assert package["live_ticker_validation_execution_digest"] == (
        review.candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_LIVE_TICKER_VALIDATION_EXECUTION_DIGEST
    )
    assert package["ticker_universe_selection_approval_digest"] == (
        review.candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_TICKER_UNIVERSE_SELECTION_APPROVAL_DIGEST
    )
    assert package["split_event_authority_candidate_review_package_digest"] == (
        review.candidate_service.EXPECTED_SPLIT_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_DIGEST
    )
    assert package["split_event_authority_candidate_digest"] == (
        review.candidate_service.EXPECTED_SPLIT_EVENT_AUTHORITY_CANDIDATE_DIGEST
    )


def test_target_universe_and_dividend_candidate_objective_are_preserved():
    package = _package()
    ...
    assert package["ready_for_dividend_event_authority_candidate"] is True
    assert package["dividend_event_authority_candidate_objective"] == (
        review.candidate_service.DIVIDEND_EVENT_AUTHORITY_CANDIDATE_OBJECTIVE
    )
    assert package["dividend_event_authority_candidate_scope"] == (
        review.candidate_service.DIVIDEND_EVENT_AUTHORITY_CANDIDATE_SCOPE
    )
    assert package["dividend_event_authority_creation_status"] == (
        review.candidate_service.DIVIDEND_EVENT_AUTHORITY_CREATION_STATUS
    )
    assert package["dividend_event_authority_freeze_status"] == (
        review.candidate_service.DIVIDEND_EVENT_AUTHORITY_FREEZE_STATUS
    )


def test_per_ticker_review_entries_are_ready_and_digest_bound():
    package = _package()
    entries = package["per_ticker_dividend_event_candidate_entries"]
    review_entries = package["per_ticker_dividend_event_review_entries"]
    ...
    for entry in review_entries:
        assert entry["dividend_event_candidate_status"] == (
            review.candidate_service.DIVIDEND_EVENT_CANDIDATE_READY_FOR_OPERATOR_REVIEW
        )
        assert entry["dividend_event_review_status"] == review.READY_FOR_OPERATOR_ASSESSMENT
        assert entry["dividend_event_authority_status"] == review.candidate_service.NOT_CREATED
        assert entry["dividend_event_freeze_status"] == review.candidate_service.NOT_FROZEN
        ...
        assert entry["dividend_history_status"] == review.candidate_service.NOT_FETCHED
        ...
        assert entry["per_ticker_dividend_event_candidate_digest"] == (
            review.candidate_service.per_ticker_dividend_event_candidate_digest_v1(
                review._candidate_entry_from_review_entry(entry)
            )
        )
        assert entry["per_ticker_dividend_event_review_digest"] == (
            review.per_ticker_dividend_event_review_digest_v1(entry)
        )
...
def test_evidence_requirements_future_policy_chains_gates_and_risks_are_reviewed():
    ...
    assert package["dividend_event_evidence_requirements"] == (
        review.DIVIDEND_EVENT_EVIDENCE_REQUIREMENTS
    )
    assert package["dividend_event_evidence_requirement_policy"] == (
        review.DIVIDEND_EVENT_EVIDENCE_REQUIREMENT_POLICY
    )
    assert package["dividend_policy_reconciliation_requirements"] == (
        review.DIVIDEND_POLICY_RECONCILIATION_REQUIREMENTS
    )
    assert package["future_dividend_provider_request_policy"] == (
        review.FUTURE_DIVIDEND_PROVIDER_REQUEST_POLICY
    )
    assert package["future_dividend_authority_chain"] == (
        review.FUTURE_DIVIDEND_AUTHORITY_CHAIN
    )
...
def test_planned_outputs_are_nine_not_generated_and_research_only():
    package = _package()
...
def test_review_created_is_the_only_created_authority_transition_flag():
    ...
    assert package["dividend_event_authority_review_created"] is True
    assert package["dividend_event_authority_candidate_created"] is True
    assert package["dividend_event_authority_created"] is False
    assert package["dividend_event_authority_frozen"] is False
    assert package["split_event_authority_candidate_created"] is True
...
def test_review_package_and_per_ticker_review_digests_are_deterministic():
    ...
    assert first["dividend_event_authority_candidate_review_package_digest"] == (
        second["dividend_event_authority_candidate_review_package_digest"]
    )
    assert first["dividend_event_authority_candidate_review_package_digest"] == (
        review.dividend_event_authority_candidate_review_package_digest_v1(first)
    )
    assert [
        entry["per_ticker_dividend_event_review_digest"]
        for entry in first["per_ticker_dividend_event_review_entries"]
    ] == [
        entry["per_ticker_dividend_event_review_digest"]
        for entry in second["per_ticker_dividend_event_review_entries"]
    ]


def test_validator_accepts_valid_review_package():
    validation = review.validate_dividend_event_authority_candidate_review_package_v1(
        _package()
    )

    assert validation["status"] == (
        "DIVIDEND_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_VALID"
    )
    ...
    assert validation["ready_for_dividend_provider_evidence_request_approval"] is False
    assert validation["ready_for_dividend_event_authority_freeze"] is False
    assert validation["dividend_event_authority_authorized"] is False


@pytest.mark.parametrize(
    ("field", "value", "match"),
    [
        ("artifact_kind", "DIVIDEND_EVENT_AUTHORITY_APPROVED", "artifact_kind"),
        ("review_status", "APPROVED", "review_status"),
        ("reviewed_dividend_event_authority_candidate_digest", "0" * 64, "candidate_digest"),
        ("dividend_event_authority_candidate_digest", "0" * 64, "candidate_digest"),
        ("reviewed_dividend_event_authority_candidate_status", "CREATED", "candidate_status"),
        ...
        ("ready_for_dividend_event_authority_candidate", False, "ready_for_dividend_event_authority_candidate"),
        ("dividend_event_authority_candidate_scope", "AUTHORITY", "dividend_event_authority_candidate_scope"),
        ("dividend_event_authority_review_created", False, "dividend_event_authority_review_created"),
        ("dividend_event_authority_candidate_created", False, "dividend_event_authority_candidate_created"),
        ...
        ("dividend_provider_evidence_results_created", True, "dividend_provider_evidence_results_created"),
        ("dividend_provider_evidence_request_approval_created", True, "dividend_provider_evidence_request_approval_created"),
        ("dividend_provider_evidence_execution_created", True, "dividend_provider_evidence_execution_created"),
        ("split_event_authority_candidate_created", False, "split_event_authority_candidate_created"),
...
def test_validator_rejects_invalid_top_level_mutations(
    ...
    with pytest.raises(
        review.DividendEventAuthorityCandidateReviewPackageError,
        match=match,
    ):
        review.validate_dividend_event_authority_candidate_review_package_v1(package)
...
@pytest.mark.parametrize(
    ("field", "match"),
    [
        ("per_ticker_dividend_event_candidate_entries", "candidate_entries"),
        ("per_ticker_dividend_event_review_entries", "review_entries"),
    ],
)
...
@pytest.mark.parametrize(
    ("digest_field", "match"),
    [
        ("per_ticker_dividend_event_candidate_digest", "candidate_digest"),
        ("per_ticker_dividend_event_review_digest", "review_digest"),
    ],
)
def test_validator_rejects_missing_per_ticker_digests(
    ...
    package = _package()
    package["per_ticker_dividend_event_review_entries"][0].pop(digest_field)
...
def test_validator_rejects_review_package_digest_mismatch():
    package = _package()
    package["dividend_event_authority_candidate_review_package_digest"] = "0" * 64
...
def test_validator_rejects_missing_review_package_digest():
    package = _package()
    package.pop("dividend_event_authority_candidate_review_package_digest")
...
def test_markdown_includes_required_sections():
    markdown = review.build_dividend_event_authority_candidate_review_markdown_v1(
        _package()
    )
...
def test_write_review_package_writes_json_without_overwrite(tmp_path: Path):
    result = review.write_dividend_event_authority_candidate_review_package_v1(tmp_path)
    ...
    with pytest.raises(
        ...
        review.write_dividend_event_authority_candidate_review_package_v1(tmp_path)


def test_services_package_exports_review_helpers():
    package = _package()

    assert services.ARTIFACT_KIND_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE == (
        review.ARTIFACT_KIND_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE
    )
    assert services.DIVIDEND_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_READY == (
        review.DIVIDEND_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_READY
    )
    assert services.build_dividend_event_authority_candidate_review_package_v1 is (
        review.build_dividend_event_authority_candidate_review_package_v1
    )
    assert services.validate_dividend_event_authority_candidate_review_package_v1 is (
        review.validate_dividend_event_authority_candidate_review_package_v1
    )
    assert services.write_dividend_event_authority_candidate_review_package_v1 is (
        review.write_dividend_event_authority_candidate_review_package_v1
    )
    assert services.build_dividend_event_authority_candidate_review_markdown_v1 is (
        review.build_dividend_event_authority_candidate_review_markdown_v1
    )
    assert services.dividend_event_authority_candidate_review_package_digest_v1(package) == (
        package["dividend_event_authority_candidate_review_package_digest"]
    )
    assert services.per_ticker_dividend_event_review_digest_v1(
        package["per_ticker_dividend_event_review_entries"][0]
    ) == package["per_ticker_dividend_event_review_entries"][0][
