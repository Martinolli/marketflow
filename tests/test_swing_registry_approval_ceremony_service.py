...
from marketflow.services import swing_registry_approval_ceremony_service as approval
...
def _attestation(**overrides) -> dict:
    values = {
        ...
        "operator_attestation_phrase": approval.REQUIRED_SWING_REGISTRY_APPROVAL_ATTESTATION_PHRASE,
        ...
        "operator_confirms_swing_frozen_digest": approval.registry.EXPECTED_SWING_FROZEN_DIGEST,
        ...
        "operator_confirms_acquisition_generation_frozen_digest": approval.registry.EXPECTED_ACQUISITION_GENERATION_FROZEN_DIGEST,
    ...
    return approval.build_swing_registry_approval_attestation_v1(**values)


def _approved() -> dict:
    return approval.build_swing_registry_approved_v1(operator_attestation=_attestation())


def test_operator_attestation_builder_creates_required_fields():
    attestation = _attestation()

    assert attestation["operator_decision"] == approval.OPERATOR_DECISION_APPROVE_SWING_REGISTRY_ENTRY
    assert attestation["operator_attestation_phrase"] == approval.REQUIRED_SWING_REGISTRY_APPROVAL_ATTESTATION_PHRASE
    assert attestation["operator_reference"] == "TEST_OPERATOR"
    assert attestation["operator_confirms_registry_review_package_digest"] == approval.EXPECTED_REGISTRY_REVIEW_PACKAGE_DIGEST
...
def test_approved_artifact_builds_offline_without_provider_calls(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        approval.review.registry.swing_freeze.swing.acquisition,
...
def test_artifact_kind_is_swing_registry_approved():
    assert _approved()["artifact_kind"] == approval.ARTIFACT_KIND_SWING_REGISTRY_APPROVED


def test_approval_status_is_swing_registry_approved():
    assert _approved()["approval_status"] == approval.SWING_REGISTRY_APPROVED


def test_registry_approval_created_is_true():
    assert _approved()["registry_approval_created"] is True


def test_registry_eligibility_is_true():
    assert _approved()["registry_eligibility"] is True


def test_registry_activation_is_true():
    assert _approved()["registry_activation"] is True
...
def test_swing_frozen_digest_matches_expected():
    assert _approved()["swing_canonical_dataset_frozen_digest"] == approval.registry.EXPECTED_SWING_FROZEN_DIGEST
...
def test_approval_summary_counts_total_passed_failed_correctly():
    ...
    assert summary["swing_registry_approval_authorized_by_operator"] is True
...
def test_operator_attestation_phrase_must_match_exactly():
    with pytest.raises(approval.SwingRegistryApprovalCeremonyError, match="operator_attestation_phrase_matches"):
        approval.build_swing_registry_approved_v1(
            operator_attestation=_attestation(operator_attestation_phrase="APPROVE SWING REGISTRY ENTRY")
        )


def test_wrong_operator_decision_is_rejected():
    with pytest.raises(approval.SwingRegistryApprovalCeremonyError, match="operator_decision_approved"):
        approval.build_swing_registry_approved_v1(operator_attestation=_attestation(operator_decision="REJECT"))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("operator_confirms_swing_frozen_digest", "0" * 64, "operator_swing_frozen_digest_confirmation_matches"),
...
def test_operator_attestation_rejects_bad_confirmations(field: str, value, match: str):
    with pytest.raises(approval.SwingRegistryApprovalCeremonyError, match=match):
        approval.build_swing_registry_approved_v1(operator_attestation=_attestation(**{field: value}))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("registry_approval_created", False, "registry_approval_created"),
        ("registry_eligibility", False, "registry_eligibility"),
        ("registry_activation", False, "registry_activation"),
...
def test_validator_rejects_invalid_approval_mutations(field: str, value, match: str):
    ...
    with pytest.raises(approval.SwingRegistryApprovalCeremonyError, match=match):
        approval.validate_swing_registry_approved_v1(artifact)
...
def test_validator_rejects_missing_operator_attestation():
    ...
    with pytest.raises(approval.SwingRegistryApprovalCeremonyError, match="operator_attestation"):
        approval.validate_swing_registry_approved_v1(artifact)


def test_validator_rejects_mutated_digest_field():
    artifact = _approved()
    artifact["swing_registry_approved_semantic_digest"] = "0" * 64

    with pytest.raises(approval.SwingRegistryApprovalCeremonyError, match="swing_registry_approved_semantic_digest"):
        approval.validate_swing_registry_approved_v1(artifact)
...
def test_approved_artifact_digest_is_deterministic():
    ...
    assert first["swing_registry_approved_semantic_digest"] == second["swing_registry_approved_semantic_digest"]
    assert first["swing_registry_approved_semantic_digest"] == approval.swing_registry_approved_semantic_digest_v1(first)


def test_remaining_roadmap_contains_required_future_work():
    roadmap = _approved()["remaining_roadmap"]

    assert roadmap == approval.REMAINING_ROADMAP_AFTER_SWING_REGISTRY_APPROVAL
...
def test_markdown_writer_includes_required_sections_and_guardrails():
    markdown = approval.build_swing_registry_approved_markdown_v1(_approved())
...
def test_write_approved_artifact_writes_json_without_overwrite(tmp_path: Path):
    result = approval.write_swing_registry_approved_v1(tmp_path, operator_attestation=_attestation())

    assert result["artifact_kind"] == approval.ARTIFACT_KIND_SWING_REGISTRY_APPROVED
    assert result["payload_sha256"]
    with pytest.raises(approval.SwingRegistryApprovalCeremonyError, match="already exists"):
        approval.write_swing_registry_approved_v1(tmp_path, operator_attestation=_attestation())


def test_registry_approval_ceremony_exports_are_public():
    import marketflow.services as services

    assert services.ARTIFACT_KIND_SWING_REGISTRY_APPROVED == "SWING_REGISTRY_APPROVED"
    assert services.SWING_REGISTRY_APPROVED == "SWING_REGISTRY_APPROVED"
    assert (
        services.REQUIRED_SWING_REGISTRY_APPROVAL_ATTESTATION_PHRASE
        == approval.REQUIRED_SWING_REGISTRY_APPROVAL_ATTESTATION_PHRASE
    )
    assert services.build_swing_registry_approval_attestation_v1 is approval.build_swing_registry_approval_attestation_v1
    assert services.build_swing_registry_approved_v1 is approval.build_swing_registry_approved_v1
    assert services.validate_swing_registry_approved_v1 is approval.validate_swing_registry_approved_v1
    assert services.write_swing_registry_approved_v1 is approval.write_swing_registry_approved_v1
    assert services.build_swing_registry_approved_markdown_v1 is approval.build_swing_registry_approved_markdown_v1
