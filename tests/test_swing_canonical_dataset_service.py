...
def _full_session_rows(session: dict, *, start_index: int = 0, vwap: str | None = "101") -> list[dict]:
    start = swing._parse_utc(session["market_open_utc"])
    return [
        _source_row(
            timestamp_utc=swing._utc_text(start + timedelta(minutes=15 * offset)),
...
def _january_2025_schedule() -> list[dict]:
    return [
        row
        for row in swing.calendar_service.build_exchange_calendar_schedule_rows_v1()
...
def _ready_candidate() -> dict:
    ...
    return swing.build_swing_canonical_dataset_candidate_v1(
        source_rows=rows,
        source_rows_digest=swing.EXPECTED_NORMALIZED_SOURCE_ROWS_DIGEST,
        fixture_mode=True,
...
def test_candidate_builds_offline_without_provider_calls(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(swing.acquisition, "fetch_massive_custom_bars_v1", fail_provider_call)

    candidate = swing.build_swing_canonical_dataset_candidate_from_local_artifact_v1("missing-root")
...
def test_candidate_binds_acquisition_frozen_digest():
    assert _ready_candidate()["acquisition_generation_frozen_digest"] == swing.EXPECTED_ACQUISITION_GENERATION_FROZEN_DIGEST
...
def test_full_ordinary_session_with_26_source_rows_produces_2_swing_bars():
    ...
    assert len(result["dataset_rows"]) == 2


def test_each_swing_bar_uses_13_source_rows():
    session = _january_2025_schedule()[0]
    result = swing.build_swing_bars_from_normalized_source_rows_v1(_full_session_rows(session), schedule_rows=[session])
...
def test_ohlc_aggregation_is_correct():
    session = _january_2025_schedule()[0]
    result = swing.build_swing_bars_from_normalized_source_rows_v1(_full_session_rows(session), schedule_rows=[session])
    first = result["dataset_rows"][0]

    assert first["open"] == "100"
    assert first["high"] == "114"
    assert first["low"] == "99"
    assert first["close"] == "113"
...
def test_volume_aggregation_is_correct():
    ...
    assert result["dataset_rows"][0]["volume"] == str(sum(range(10, 23)))


def test_vwap_aggregation_is_deterministic_or_null_when_unavailable():
    session = _january_2025_schedule()[0]
    with_vwap = swing.build_swing_bars_from_normalized_source_rows_v1(_full_session_rows(session), schedule_rows=[session])
    without_vwap = swing.build_swing_bars_from_normalized_source_rows_v1(
...
def test_bar_at_exact_session_close_is_not_included_in_rth_source_rows():
    ...
    close = swing._parse_utc(session["market_close_utc"])
    rows.append(_source_row(timestamp_utc=swing._utc_text(close), index=99, session_date=session["session_date"]))

    result = swing.build_swing_bars_from_normalized_source_rows_v1(rows, schedule_rows=[session])
...
def test_candidate_digest_is_deterministic():
    assert _ready_candidate()["swing_candidate_semantic_digest"] == _ready_candidate()["swing_candidate_semantic_digest"]


def test_validator_accepts_valid_ready_candidate():
    validation = swing.validate_swing_canonical_dataset_candidate_v1(_ready_candidate())
...
def test_validator_rejects_invalid_candidate_fields(field: str, value, match: str):
    ...
    with pytest.raises(swing.SwingCanonicalDatasetError, match=match):
        swing.validate_swing_canonical_dataset_candidate_v1(candidate)


def test_validator_rejects_2025_01_swing_cross_check_mismatch():
    candidate = _ready_candidate()
    candidate["2025_01_swing_cross_check"]["actual_swing_bars"] = 39

    with pytest.raises(swing.SwingCanonicalDatasetError, match="2025-01 SWING cross-check mismatch"):
        swing.validate_swing_canonical_dataset_candidate_v1(candidate)


def test_validator_rejects_full_session_bar_with_wrong_source_row_count():
    candidate = _ready_candidate()
    candidate["dataset_rows"][0]["source_row_count"] = 12

    with pytest.raises(swing.SwingCanonicalDatasetError, match="source_row_count"):
        swing.validate_swing_canonical_dataset_candidate_v1(candidate)


def test_validator_accepts_blocked_missing_source_candidate():
    candidate = swing.build_swing_canonical_dataset_candidate_v1()
    validation = swing.validate_swing_canonical_dataset_candidate_v1(candidate)
...
def test_writer_writes_only_ignored_dataset_output_and_not_raw_provider_payload(tmp_path: Path):
    candidate = _ready_candidate()
    output_dir = tmp_path / ".marketflow" / "canonical_candidates" / "AAPL" / "SWING"

    result = swing.write_swing_canonical_dataset_candidate_outputs_v1(output_dir, candidate=candidate)
...
def test_source_row_artifact_writer_requires_ignored_output(tmp_path: Path):
    rows, _schedule = _january_2025_source_rows()

    with pytest.raises(swing.SwingCanonicalDatasetError, match=".marketflow"):
        swing.write_frozen_acquisition_normalized_source_rows_artifact_v1(tmp_path / "not_ignored", rows)
