...
from marketflow.services import (
    predictive_experiment_execution_candidate_operator_review_service as review_service,
)
...
def _review_package() -> dict[str, Any]:
    return review_service.build_predictive_experiment_execution_candidate_review_package_v1()
...
def test_review_package_builds_offline_without_provider_calls(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        review_service.candidate_service.plan_review_service.plan_service.acquisition,
...
def test_artifact_kind_and_status_are_execution_candidate_review_package():
    review_package = _review_package()

    assert review_package["artifact_kind"] == (
        review_service.ARTIFACT_KIND_PREDICTIVE_EXPERIMENT_EXECUTION_CANDIDATE_REVIEW_PACKAGE
    )
    assert review_package["review_status"] == (
        review_service.PREDICTIVE_EXPERIMENT_EXECUTION_CANDIDATE_REVIEW_PACKAGE_READY
    )


def test_reviewed_execution_candidate_digest_and_request_are_bound():
    review_package = _review_package()

    assert review_package["reviewed_execution_candidate_digest"] == (
        review_service.EXPECTED_PREDICTIVE_EXPERIMENT_EXECUTION_CANDIDATE_DIGEST
    )
    assert review_package["reviewed_execution_request_id"] == (
        "AAPL_PREDICTIVE_EXPERIMENT_EXECUTION_2022_2025_V1"
    )
    assert review_package["predictive_experiment_execution_request_id"] == (
        "AAPL_PREDICTIVE_EXPERIMENT_EXECUTION_2022_2025_V1"
    )
...
def test_source_evidence_digests_are_bound():
    ...
    assert review_package["predictive_experiment_plan_digest"] == (
        review_service.candidate_service.EXPECTED_PREDICTIVE_EXPERIMENT_PLAN_DIGEST
    )
    assert review_package["predictive_experiment_plan_review_package_digest"] == (
        review_service.candidate_service.EXPECTED_PREDICTIVE_EXPERIMENT_PLAN_REVIEW_PACKAGE_DIGEST
    )
    assert review_package["predictive_usefulness_review_candidate_digest"] == (
        review_service.candidate_service.plan_review_service.plan_service.EXPECTED_PREDICTIVE_USEFULNESS_REVIEW_CANDIDATE_DIGEST
    )
    assert review_package["predictive_usefulness_review_candidate_review_package_digest"] == (
        review_service.candidate_service.plan_review_service.plan_service.EXPECTED_PREDICTIVE_USEFULNESS_REVIEW_CANDIDATE_REVIEW_PACKAGE_DIGEST
    )
    assert review_package["campaign_execution_results_review_package_digest"] == (
        review_service.candidate_service.plan_review_service.plan_service.EXPECTED_CAMPAIGN_EXECUTION_RESULTS_REVIEW_PACKAGE_DIGEST
    )
    assert review_package["campaign_execution_digest"] == (
        review_service.candidate_service.plan_review_service.plan_service.EXPECTED_CAMPAIGN_EXECUTION_DIGEST
    )
    assert review_package["swing_registry_approval_digest"] == (
        review_service.candidate_service.plan_review_service.plan_service.EXPECTED_SWING_REGISTRY_APPROVAL_DIGEST
    )
    assert review_package["position_swing_registry_approval_digest"] == (
        review_service.candidate_service.plan_review_service.plan_service.EXPECTED_POSITION_SWING_REGISTRY_APPROVAL_DIGEST
    )


def test_execution_request_scope_is_preserved():
    review_package = _review_package()
    ...
    assert review_package["execution_mode"] == "OFFLINE_RESEARCH_EXPERIMENT"
    assert review_package["runtime_mode"] == "NOT_RUNTIME"
    assert review_package["strategy_mode"] == "NOT_STRATEGY_INPUT"
    assert review_package["broker_mode"] == "DISABLED"
    assert review_package["paper_trading_mode"] == "DISABLED"


def test_planned_inputs_labels_features_and_splits_are_preserved():
    review_package = _review_package()

    assert [item["profile"] for item in review_package["planned_input_files"]] == [
        "SWING",
        "POSITION_SWING",
    ]
    assert [item["label"] for item in review_package["label_definitions"]] == (
        review_service.candidate_service.LABEL_DEFINITIONS
    )
    assert [item["feature_family"] for item in review_package["feature_family_plan"]] == (
        review_service.candidate_service.FEATURE_FAMILIES
    )
    assert review_package["walk_forward_plan"]["method"] == "chronological_walk_forward"
    assert review_package["walk_forward_plan"]["training_window"] == "planned"
    assert review_package["walk_forward_plan"]["validation_window"] == "planned"
    assert review_package["walk_forward_plan"]["test_window"] == "planned"
    assert review_package["walk_forward_plan"]["no_shuffle"] is True
    assert review_package["walk_forward_plan"]["time_order_preserved"] is True
    assert review_package["out_of_sample_plan"]["final_holdout_period"] == "planned"
...
def test_planned_outputs_are_not_generated_and_research_only():
    ...
    assert review_package["planned_output_count"] == 13
    assert review_package["planned_outputs_status"] == review_service.candidate_service.PLANNED_NOT_GENERATED
    assert review_package["planned_outputs_label"] == (
        review_service.candidate_service.RESEARCH_ONLY_NON_ACTIONABLE
    )
    assert {item["generation_status"] for item in review_package["planned_outputs"]} == {
        review_service.candidate_service.PLANNED_NOT_GENERATED
    }
    assert {item["output_label"] for item in review_package["planned_outputs"]} == {
        review_service.candidate_service.RESEARCH_ONLY_NON_ACTIONABLE
    }
...
def test_execution_gates_and_risk_controls_are_defined():
    ...
    assert review_package["execution_gates"] == review_service.candidate_service.EXECUTION_GATES
...
def test_execution_acceptance_and_runtime_boundaries_remain_closed():
    ...
    assert review_package["label_generation_performed"] is False
    assert review_package["feature_matrix_generation_performed"] is False
    ...
    assert review_package["predictive_usefulness"] == (
        review_service.candidate_service.plan_review_service.plan_service.acquisition.PREDICTIVE_USEFULNESS_NOT_ACCEPTED
    )
    assert review_package["predictive_usefulness_acceptance_ready"] is False
    assert review_package["profitability"] == (
        review_service.candidate_service.plan_review_service.plan_service.acquisition.PROFITABILITY_NOT_ACCEPTED
    )
...
def test_summary_counts_total_passed_failed_and_blockers():
    ...
    assert summary["ready_for_predictive_experiment_execution_approval"] is True
...
def test_review_package_digest_is_deterministic():
    ...
    assert first["predictive_experiment_execution_candidate_review_package_digest"] == (
        second["predictive_experiment_execution_candidate_review_package_digest"]
    )
    assert first["predictive_experiment_execution_candidate_review_package_digest"] == (
        EXPECTED_REVIEW_PACKAGE_DIGEST
    )
    assert first["predictive_experiment_execution_candidate_review_package_digest"] == (
        review_service.predictive_experiment_execution_candidate_review_package_digest_v1(
            first
        )
    )
...
def test_validator_accepts_valid_review_package():
    validation = (
        review_service.validate_predictive_experiment_execution_candidate_review_package_v1(
            _review_package()
        )
    )

    assert validation["status"] == (
        "PREDICTIVE_EXPERIMENT_EXECUTION_CANDIDATE_REVIEW_PACKAGE_VALID"
    )
    assert validation["ready_for_operator_assessment"] is True
    assert validation["ready_for_predictive_experiment_execution_approval"] is True
...
@pytest.mark.parametrize(
    ("field", "value", "match"),
    [
        ("reviewed_execution_candidate_digest", "0" * 64, "reviewed_execution_candidate_digest"),
        ("reviewed_execution_request_id", "WRONG", "reviewed_execution_request_id"),
...
def test_validator_rejects_forbidden_review_package_mutations(
    ...
    with pytest.raises(
        review_service.PredictiveExperimentExecutionCandidateReviewPackageError,
        match=match,
    ):
        review_service.validate_predictive_experiment_execution_candidate_review_package_v1(
            review_package
...
def test_validator_rejects_non_aapl_ticker_universe():
    review_package = _mutated_review_package("ticker_universe", ["AAPL", "MSFT"])
...
def test_validator_rejects_missing_swing_profile():
    review_package = _review_package()
    review_package["dataset_profiles"] = [
        item for item in review_package["dataset_profiles"] if item["profile"] != "SWING"
    ]
...
def test_validator_rejects_missing_position_swing_profile():
    ...
    review_package["dataset_profiles"] = [
        item
        for item in review_package["dataset_profiles"]
...
def test_validator_rejects_missing_predictive_experiment_plan_review_digest():
    review_package = _mutated_review_package(
...
def test_validator_rejects_missing_execution_gates():
    review_package = _mutated_review_package("execution_gates", [])
...
def test_validator_rejects_missing_risk_controls():
    review_package = _mutated_review_package("risk_controls", [])
...
def test_validator_rejects_missing_review_package_digest():
    review_package = _review_package()
    review_package.pop("predictive_experiment_execution_candidate_review_package_digest")

    with pytest.raises(
        review_service.PredictiveExperimentExecutionCandidateReviewPackageError,
        match="predictive_experiment_execution_candidate_review_package_digest",
    ):
        review_service.validate_predictive_experiment_execution_candidate_review_package_v1(
            review_package
        )
...
def test_markdown_writer_includes_required_sections():
    markdown = (
        review_service.build_predictive_experiment_execution_candidate_review_markdown_v1(
            _review_package()
        )
    )
...
def test_write_review_package_writes_json_without_overwrite(tmp_path: Path):
    result = (
        review_service.write_predictive_experiment_execution_candidate_review_package_v1(
            tmp_path
        )
    )
...
def test_execution_candidate_review_service_exports_are_public():
    import marketflow.services as services

    assert services.ARTIFACT_KIND_PREDICTIVE_EXPERIMENT_EXECUTION_CANDIDATE_REVIEW_PACKAGE == (
        review_service.ARTIFACT_KIND_PREDICTIVE_EXPERIMENT_EXECUTION_CANDIDATE_REVIEW_PACKAGE
    )
    assert services.PREDICTIVE_EXPERIMENT_EXECUTION_CANDIDATE_REVIEW_PACKAGE_READY == (
        review_service.PREDICTIVE_EXPERIMENT_EXECUTION_CANDIDATE_REVIEW_PACKAGE_READY
    )
    assert services.build_predictive_experiment_execution_candidate_review_package_v1 is (
        review_service.build_predictive_experiment_execution_candidate_review_package_v1
    )
    assert services.validate_predictive_experiment_execution_candidate_review_package_v1 is (
        review_service.validate_predictive_experiment_execution_candidate_review_package_v1
    )
    assert services.write_predictive_experiment_execution_candidate_review_package_v1 is (
        review_service.write_predictive_experiment_execution_candidate_review_package_v1
    )
    assert services.build_predictive_experiment_execution_candidate_review_markdown_v1 is (
        review_service.build_predictive_experiment_execution_candidate_review_markdown_v1
    )


def test_object_binding_mode_is_recorded_when_candidate_is_supplied():
    candidate = review_service.candidate_service.build_predictive_experiment_execution_candidate_v1()
    ...
    assert review_package["candidate_binding_mode"] == (
        review_service.PREDICTIVE_EXPERIMENT_EXECUTION_CANDIDATE_OBJECT_BINDING
    )
