...
- A comprehensive, interactive Candlestick chart with Wyckoff Phases and Events.
...
def plot_point_and_figure(df, output_dir, box_size=None, reversal=3, wyckoff_overlay=False):
    ...
    if box_size is None:
        if 'atr14' in df.columns and pd.notna(df['atr14'].mean()) and df['atr14'].mean() > 0:
            box_size = round(df['atr14'].mean(), 2)
            logger.info(f"Auto-detected box_size based on ATR: {box_size}")
        else:
            avg_spread = (df['high'] - df['low']).mean()
            box_size = round(avg_spread, 2) if pd.notna(avg_spread) and avg_spread > 0 else 1.0
...
def plot_features(csv_file, features=None, nrows=4000, box_size=None, reversal=3):
    ...
    # --- MODIFICATION START ---
    # Convert timestamp to datetime objects for Plotly
    ...
    # The old price/volume chart is now replaced by the new candlestick chart.
    # The following individual plots can still be useful for isolated analysis.

    # Plot Volume over Time
    fig = px.histogram(df, x= "timestamp", y='volume', nbins=150,
                       title=f"Volume Distribution Over Time {csv_file_name}",
    ...
    fig = px.line(df, x='timestamp', y='spread',
                  title=f"Spread Over Time {csv_file_name}",
    ...
    if "volume_class" in features:
        fig = px.scatter(df, x='timestamp', y='close',
                      color='volume_class',
                      title=f"Volume Class Over Time {csv_file_name}",
    ...
    if "candle_class" in features:
        ...
        fig = px.scatter(df, x='timestamp', y='close', color='candle_class',
                         title=f"Classified Candles {csv_file_name}",
    ...
    if "price_direction" in features:
        ...
        fig = px.scatter(df, x='timestamp', y='close', color='price_direction',
                         title=f"Price Direction {csv_file_name}",
                         labels={'Index (row)': 'Index (row)', 'close': 'Closed Price'},
                         color_discrete_sequence=px.colors.qualitative.Plotly)
    ...
    if "volume_direction" in features:
        ...
        fig = px.scatter(df, x='timestamp', y='close', color='volume_direction',
                         title=f"Volume Direction {csv_file_name}",
                         labels={'Index (row)': 'Index (row)', 'close': 'Closed Price'},
                         color_discrete_sequence=px.colors.qualitative.Plotly)
