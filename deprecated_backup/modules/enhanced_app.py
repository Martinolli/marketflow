...
def enhanced_interactive_mode(interface: EnhancedMarketFlowInterface):
    ...
    print("\n" + "="*80)
    print("🚀 ENHANCED MARKETFLOW INTERACTIVE MODE")
    print("="*80)
    ...
    print("  ⚙️  context             - Show current context")
    ...
    while True:
        try:
            ...
            prompt = f"{context_str}MarketFlow> "
            
            user_input = input(prompt).strip()
            
        except (KeyboardInterrupt, EOFError):
            print("\n👋 Exiting Enhanced MarketFlow...")
            break
        ...
        if user_input.lower().startswith('ticker '):
            ticker = user_input[7:].strip().upper()
            interface.update_context(ticker=ticker)
            print(f"✅ Default ticker set to: {ticker}")
            continue
        
        if user_input.lower().startswith('timeframes '):
            timeframes = user_input[11:].strip().split()
            interface.update_context(timeframes=timeframes)
            print(f"✅ Default timeframes set to: {', '.join(timeframes)}")
            continue
        
        if user_input.lower() == 'clear':
            try:
                interface.query_engine.clear_memory()
