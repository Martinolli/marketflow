"""Offline operator review package for the dividend-event authority candidate."""
...
from marketflow.services import dividend_event_authority_candidate_service as candidate_service


ARTIFACT_KIND_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE = (
    "DIVIDEND_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE"
)
SCHEMA_VERSION_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_REVIEW_V1 = (
    "dividend_event_authority_candidate_review_v1"
)
DIVIDEND_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_READY = (
    "DIVIDEND_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_READY"
)
DIVIDEND_EVENT_AUTHORITY_CANDIDATE_STATUS_BINDING = (
    "DIVIDEND_EVENT_AUTHORITY_CANDIDATE_STATUS_BINDING"
)
DIVIDEND_EVENT_AUTHORITY_CANDIDATE_OBJECT_BINDING = (
    "DIVIDEND_EVENT_AUTHORITY_CANDIDATE_OBJECT_BINDING"
)

EXPECTED_REVIEWED_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_DIGEST = (
    "44cabaebea32b4d618d13c4e1c77190c2549b9c15c8481460ab66211d1f44097"
)
EXPECTED_REVIEWED_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_CHECKLIST_TOTAL = len(
    candidate_service.REQUIRED_CHECK_IDS
)
EXPECTED_REVIEWED_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_CHECKLIST_PASSED = len(
    candidate_service.REQUIRED_CHECK_IDS
)
EXPECTED_REVIEWED_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_CHECKLIST_FAILED = 0
EXPECTED_REVIEWED_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_BLOCKER_COUNT = 0
...
DIVIDEND_EVENT_EVIDENCE_REQUIREMENTS = list(
    candidate_service.DIVIDEND_EVENT_EVIDENCE_REQUIREMENTS
)
DIVIDEND_EVENT_EVIDENCE_REQUIREMENT_POLICY = deepcopy(
    candidate_service.DIVIDEND_EVENT_EVIDENCE_REQUIREMENT_POLICY
)
DIVIDEND_POLICY_RECONCILIATION_REQUIREMENTS = list(
    candidate_service.DIVIDEND_POLICY_RECONCILIATION_REQUIREMENTS
)
FUTURE_DIVIDEND_PROVIDER_REQUEST_POLICY = deepcopy(
    candidate_service.FUTURE_DIVIDEND_PROVIDER_REQUEST_POLICY
)
FUTURE_DIVIDEND_AUTHORITY_CHAIN = list(candidate_service.FUTURE_DIVIDEND_AUTHORITY_CHAIN)
...
PLANNED_REVIEW_OUTPUT_NAMES = [
    "dividend_provider_evidence_request_approval_ceremony",
    "dividend_provider_evidence_execution",
    "dividend_event_evidence_results_review_package",
    "dividend_policy_reconciliation_review",
    "dividend_event_authority_candidate_update_or_discrepancy_triage",
    "dividend_event_authority_freeze_ceremony",
    "split_event_authority_evidence_review_or_freeze_chain",
    "combined_corporate_action_readiness_review",
    "acquisition_generation_chain_candidate_after_corporate_action_authority",
]

REQUIRED_CHECK_IDS = [
    "reviewed_dividend_event_candidate_kind_matches",
    "reviewed_dividend_event_candidate_status_ready",
    "reviewed_dividend_event_candidate_digest_matches",
    "reviewed_dividend_event_candidate_checklist_zero_blockers",
    "dividend_event_candidate_digest_bound",
    ...
    "dividend_event_candidate_objective_reviewed",
    "dividend_event_candidate_scope_candidate_only",
    "dividend_event_authority_creation_status_not_created",
    "dividend_event_authority_freeze_status_not_frozen",
    "per_ticker_dividend_event_candidate_entries_12",
    "per_ticker_dividend_event_review_entries_12",
    "per_ticker_dividend_event_candidate_status_ready",
    "per_ticker_dividend_event_review_status_ready",
    "per_ticker_dividend_event_authority_not_created",
    "per_ticker_dividend_event_freeze_not_frozen",
    "per_ticker_provider_evidence_request_not_authorized",
    "per_ticker_provider_evidence_execution_not_executed",
    "per_ticker_dividend_history_not_fetched",
    "per_ticker_dividend_event_candidate_digests_present",
    "per_ticker_dividend_event_review_digests_present",
    "dividend_event_evidence_requirements_reviewed",
    "dividend_event_evidence_requirement_policy_reviewed",
    "dividend_policy_reconciliation_requirements_reviewed",
    "future_dividend_provider_request_policy_reviewed",
    "future_dividend_authority_chain_reviewed",
    ...
    "dividend_event_authority_candidate_created_true",
    "dividend_event_authority_review_created_true",
    ...
    "dividend_provider_evidence_request_approval_created_false",
    "dividend_provider_evidence_execution_created_false",
    "split_event_authority_candidate_created_true",
    ...
    "no_dividend_provider_evidence_request_approval_created",
...
class DividendEventAuthorityCandidateReviewPackageError(ValueError):
    """Raised when the dividend-event authority candidate review package is invalid."""
...
def _expect(actual: Any, expected: Any, field_name: str) -> None:
    if actual != expected:
        raise DividendEventAuthorityCandidateReviewPackageError(f"{field_name} mismatch")


def _expect_true(actual: Any, field_name: str) -> None:
    if actual is not True:
        raise DividendEventAuthorityCandidateReviewPackageError(
...
def _candidate_for_binding(
    ...
    if candidate is None:
        return (
            candidate_service.build_dividend_event_authority_candidate_v1(),
            DIVIDEND_EVENT_AUTHORITY_CANDIDATE_STATUS_BINDING,
        )
    candidate_service.validate_dividend_event_authority_candidate_v1(candidate)
    return deepcopy(candidate), DIVIDEND_EVENT_AUTHORITY_CANDIDATE_OBJECT_BINDING


def _per_ticker_review_digest_payload(entry: dict[str, Any]) -> dict[str, Any]:
    payload = deepcopy(entry)
    payload.pop("per_ticker_dividend_event_review_digest", None)
    return payload


def per_ticker_dividend_event_review_digest_v1(entry: dict[str, Any]) -> str:
...
def _review_entries(candidate: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for source in candidate.get("per_ticker_dividend_event_candidate_entries", []):
        entry = deepcopy(source)
        entry["dividend_event_review_status"] = READY_FOR_OPERATOR_ASSESSMENT
        entry["per_ticker_dividend_event_review_digest"] = (
            per_ticker_dividend_event_review_digest_v1(entry)
        )
...
def _candidate_entry_from_review_entry(entry: dict[str, Any]) -> dict[str, Any]:
    payload = deepcopy(entry)
    payload.pop("dividend_event_review_status", None)
    payload.pop("per_ticker_dividend_event_review_digest", None)
    return payload
...
def _base_review_package(
    ...
    return {
        "artifact_kind": ARTIFACT_KIND_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE,
        "schema_version": SCHEMA_VERSION_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_REVIEW_V1,
        "review_status": DIVIDEND_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_READY,
        "dividend_event_authority_candidate_binding_mode": binding_mode,
        ...
        "dividend_provider_evidence_request_approval_created": False,
        ...
        "reviewed_dividend_event_authority_candidate_kind": candidate["artifact_kind"],
        "reviewed_dividend_event_authority_candidate_status": candidate["candidate_status"],
        "reviewed_dividend_event_authority_candidate_digest": candidate[
            "dividend_event_authority_candidate_digest"
        ],
        "reviewed_dividend_event_authority_candidate_checklist_total": summary[
            "total_checks"
        ],
        "reviewed_dividend_event_authority_candidate_checklist_passed": summary[
            "passed_checks"
        ],
        "reviewed_dividend_event_authority_candidate_checklist_failed": summary[
            "failed_checks"
        ],
        "reviewed_dividend_event_authority_candidate_blocker_count": summary[
            "blocker_count"
        ],
        "dividend_event_authority_candidate_digest": candidate[
            "dividend_event_authority_candidate_digest"
        ],
        ...
        "source_split_event_authority_candidate_review_package_kind": candidate[
            "source_split_event_authority_candidate_review_package_kind"
        ],
        "source_split_event_authority_candidate_review_package_status": candidate[
            "source_split_event_authority_candidate_review_package_status"
        ],
        "split_event_authority_candidate_review_package_digest": candidate[
        ...
        "dividend_event_authority_candidate_objective": candidate[
            "dividend_event_authority_candidate_objective"
        ],
        "dividend_event_authority_candidate_scope": candidate[
            "dividend_event_authority_candidate_scope"
        ],
        "dividend_event_authority_creation_status": candidate[
            "dividend_event_authority_creation_status"
        ],
        "dividend_event_authority_freeze_status": candidate[
            "dividend_event_authority_freeze_status"
        ],
        "per_ticker_dividend_event_candidate_entries": deepcopy(
            candidate["per_ticker_dividend_event_candidate_entries"]
        ),
        "per_ticker_dividend_event_review_entries": _review_entries(candidate),
        "dividend_event_evidence_requirements": list(
            candidate["dividend_event_evidence_requirements"]
        ),
        "dividend_event_evidence_requirement_policy": deepcopy(
            candidate["dividend_event_evidence_requirement_policy"]
        ),
        "dividend_policy_reconciliation_requirements": list(
            candidate["dividend_policy_reconciliation_requirements"]
        ),
        "future_dividend_provider_request_policy": deepcopy(
            candidate["future_dividend_provider_request_policy"]
        ),
        "future_dividend_authority_chain": list(
            candidate["future_dividend_authority_chain"]
        ),
        "future_corporate_action_readiness_chain": list(
...
def _entries(review_package: dict[str, Any]) -> list[dict[str, Any]]:
    entries = review_package.get("per_ticker_dividend_event_candidate_entries")
...
def _review_entries_from_package(
    ...
    entries = review_package.get("per_ticker_dividend_event_review_entries")
    return entries if isinstance(entries, list) else []
...
def _checklist(review_package: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        _check("reviewed_dividend_event_candidate_kind_matches", candidate_service.ARTIFACT_KIND_DIVIDEND_EVENT_AUTHORITY_CANDIDATE, review_package.get("reviewed_dividend_event_authority_candidate_kind")),
        _check("reviewed_dividend_event_candidate_status_ready", candidate_service.DIVIDEND_EVENT_AUTHORITY_READY_FOR_OPERATOR_REVIEW, review_package.get("reviewed_dividend_event_authority_candidate_status")),
        _check("reviewed_dividend_event_candidate_digest_matches", EXPECTED_REVIEWED_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_DIGEST, review_package.get("reviewed_dividend_event_authority_candidate_digest")),
        _check("reviewed_dividend_event_candidate_checklist_zero_blockers", 0, review_package.get("reviewed_dividend_event_authority_candidate_blocker_count")),
        _check("dividend_event_candidate_digest_bound", EXPECTED_REVIEWED_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_DIGEST, review_package.get("dividend_event_authority_candidate_digest")),
        ...
        _check("registry_inventory_approval_digest_bound", candidate_service.split_review.candidate_service.approval.review.plan.EXPECTED_POST_IDENTITY_FREEZE_REGISTRY_INVENTORY_APPROVAL_DIGEST, review_package.get("post_identity_freeze_registry_inventory_approval_digest")),
        _check("registry_inventory_review_digest_bound", candidate_service.split_review.candidate_service.approval.review.plan.approval_service.EXPECTED_POST_IDENTITY_FREEZE_REGISTRY_INVENTORY_CANDIDATE_REVIEW_PACKAGE_DIGEST, review_package.get("post_identity_freeze_registry_inventory_candidate_review_package_digest")),
        _check("registry_inventory_candidate_digest_bound", candidate_service.split_review.candidate_service.approval.review.plan.approval_service.EXPECTED_POST_IDENTITY_FREEZE_REGISTRY_INVENTORY_CANDIDATE_DIGEST, review_package.get("post_identity_freeze_registry_inventory_candidate_digest")),
        _check("identity_freeze_digest_bound", candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.EXPECTED_IDENTITY_AUTHORITY_FREEZE_DIGEST, review_package.get("identity_authority_freeze_digest")),
        _check("identity_candidate_review_digest_bound", candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_IDENTITY_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_DIGEST, review_package.get("identity_authority_candidate_review_package_digest")),
        _check("identity_candidate_digest_bound", candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_IDENTITY_AUTHORITY_CANDIDATE_DIGEST, review_package.get("identity_authority_candidate_digest")),
        _check("live_validation_results_review_digest_bound", candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_LIVE_TICKER_VALIDATION_RESULTS_REVIEW_PACKAGE_DIGEST, review_package.get("live_ticker_validation_results_review_package_digest")),
        _check("live_validation_execution_digest_bound", candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_LIVE_TICKER_VALIDATION_EXECUTION_DIGEST, review_package.get("live_ticker_validation_execution_digest")),
        _check("ticker_universe_selection_approval_digest_bound", candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_TICKER_UNIVERSE_SELECTION_APPROVAL_DIGEST, review_package.get("ticker_universe_selection_approval_digest")),
        _check("target_universe_count_12", 12, review_package.get("target_universe_count")),
        _check("target_universe_matches_candidate_universe", True, review_package.get("target_universe") == review_package.get("corporate_action_plan_universe") == VALIDATION_TARGET_UNIVERSE),
        ...
        _check("ready_for_dividend_event_authority_candidate_true", True, review_package.get("ready_for_dividend_event_authority_candidate")),
        _check("dividend_event_candidate_objective_reviewed", candidate_service.DIVIDEND_EVENT_AUTHORITY_CANDIDATE_OBJECTIVE, review_package.get("dividend_event_authority_candidate_objective")),
        _check("dividend_event_candidate_scope_candidate_only", candidate_service.DIVIDEND_EVENT_AUTHORITY_CANDIDATE_SCOPE, review_package.get("dividend_event_authority_candidate_scope")),
        _check("dividend_event_authority_creation_status_not_created", candidate_service.DIVIDEND_EVENT_AUTHORITY_CREATION_STATUS, review_package.get("dividend_event_authority_creation_status")),
        _check("dividend_event_authority_freeze_status_not_frozen", candidate_service.DIVIDEND_EVENT_AUTHORITY_FREEZE_STATUS, review_package.get("dividend_event_authority_freeze_status")),
        _check("per_ticker_dividend_event_candidate_entries_12", 12, len(entries)),
        _check("per_ticker_dividend_event_review_entries_12", 12, len(review_entries)),
        _check("per_ticker_dividend_event_candidate_status_ready", True, _all_entry_field(review_entries, "dividend_event_candidate_status", candidate_service.DIVIDEND_EVENT_CANDIDATE_READY_FOR_OPERATOR_REVIEW)),
        _check("per_ticker_dividend_event_review_status_ready", True, _all_entry_field(review_entries, "dividend_event_review_status", READY_FOR_OPERATOR_ASSESSMENT)),
        _check("per_ticker_dividend_event_authority_not_created", True, _all_entry_field(review_entries, "dividend_event_authority_status", candidate_service.NOT_CREATED)),
        _check("per_ticker_dividend_event_freeze_not_frozen", True, _all_entry_field(review_entries, "dividend_event_freeze_status", candidate_service.NOT_FROZEN)),
        ...
        _check("per_ticker_dividend_history_not_fetched", True, _all_entry_field(review_entries, "dividend_history_status", candidate_service.NOT_FETCHED)),
        _check("per_ticker_dividend_event_candidate_digests_present", True, _digests_present(review_entries, "per_ticker_dividend_event_candidate_digest")),
        _check("per_ticker_dividend_event_review_digests_present", True, _digests_present(review_entries, "per_ticker_dividend_event_review_digest")),
        _check("dividend_event_evidence_requirements_reviewed", DIVIDEND_EVENT_EVIDENCE_REQUIREMENTS, review_package.get("dividend_event_evidence_requirements")),
        _check("dividend_event_evidence_requirement_policy_reviewed", DIVIDEND_EVENT_EVIDENCE_REQUIREMENT_POLICY, review_package.get("dividend_event_evidence_requirement_policy")),
        _check("dividend_policy_reconciliation_requirements_reviewed", DIVIDEND_POLICY_RECONCILIATION_REQUIREMENTS, review_package.get("dividend_policy_reconciliation_requirements")),
        _check("future_dividend_provider_request_policy_reviewed", FUTURE_DIVIDEND_PROVIDER_REQUEST_POLICY, review_package.get("future_dividend_provider_request_policy")),
        _check("future_dividend_authority_chain_reviewed", FUTURE_DIVIDEND_AUTHORITY_CHAIN, review_package.get("future_dividend_authority_chain")),
        ...
        _check("dividend_event_authority_candidate_created_true", True, review_package.get("dividend_event_authority_candidate_created")),
        _check("dividend_event_authority_review_created_true", True, review_package.get("dividend_event_authority_review_created")),
        ...
        _check("dividend_provider_evidence_request_authorized_false", False, review_package.get("dividend_provider_evidence_request_authorized")),
        _check("dividend_provider_evidence_executed_false", False, review_package.get("dividend_provider_evidence_executed")),
        _check("dividend_provider_evidence_results_created_false", False, review_package.get("dividend_provider_evidence_results_created")),
        _check("dividend_provider_evidence_request_approval_created_false", False, review_package.get("dividend_provider_evidence_request_approval_created")),
        _check("dividend_provider_evidence_execution_created_false", False, review_package.get("dividend_provider_evidence_execution_created")),
        _check("split_event_authority_candidate_created_true", True, review_package.get("split_event_authority_candidate_created")),
        ...
        _check("no_dividend_event_authority_freeze_created", False, review_package.get("dividend_event_authority_freeze_created")),
        _check("no_dividend_provider_evidence_request_approval_created", False, review_package.get("dividend_provider_evidence_request_approval_created")),
        _check("no_dividend_provider_evidence_execution_created", False, review_package.get("dividend_provider_evidence_execution_created")),
...
def _review_package_digest_payload(review_package: dict[str, Any]) -> dict[str, Any]:
    ...
    payload.pop("dividend_event_authority_candidate_review_package_digest", None)
    return payload


def dividend_event_authority_candidate_review_package_digest_v1(
    review_package: dict[str, Any],
) -> str:
...
def build_dividend_event_authority_candidate_review_package_v1(
    candidate: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Build the offline dividend-event authority candidate review package."""
    ...
    review_package["dividend_event_authority_candidate_review_package_digest"] = (
        dividend_event_authority_candidate_review_package_digest_v1(review_package)
    )
    validate_dividend_event_authority_candidate_review_package_v1(review_package)
...
def _reject_forbidden_values(
    ...
    forbidden_strings = {
        ...
        "DIVIDEND_EVENT_PROVIDER_EVIDENCE_RESULTS",
    ...
    forbidden_true_fields = {
        ...
        "dividend_provider_evidence_request_approval_created",
...
def _validate_review_entries(review_package: dict[str, Any]) -> None:
    ...
    if len(candidate_entries) != 12:
        raise DividendEventAuthorityCandidateReviewPackageError(
            "per_ticker_dividend_event_candidate_entries mismatch"
        )
    if len(review_entries) != 12:
        raise DividendEventAuthorityCandidateReviewPackageError(
            "per_ticker_dividend_event_review_entries mismatch"
        )
    _expect(
        [entry.get("ticker") for entry in candidate_entries],
        VALIDATION_TARGET_UNIVERSE,
        "per_ticker_dividend_event_candidate_entries tickers",
    )
    _expect(
        [entry.get("ticker") for entry in review_entries],
        VALIDATION_TARGET_UNIVERSE,
        "per_ticker_dividend_event_review_entries tickers",
    )
    for entry in review_entries:
        ticker = entry.get("ticker")
        for field, expected in {
            "identity_authority_status": candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.IDENTITY_FREEZE_STATUS_FROZEN,
            "registry_inventory_status": candidate_service.split_review.candidate_service.approval.review.plan.approval_service.APPROVED_FOR_FUTURE_CORPORATE_ACTION_PLANNING_ONLY,
            "corporate_action_plan_status": candidate_service.split_review.candidate_service.approval.APPROVED_FOR_FUTURE_SPLIT_AND_DIVIDEND_AUTHORITY_CANDIDATES_ONLY,
            "dividend_event_candidate_status": candidate_service.DIVIDEND_EVENT_CANDIDATE_READY_FOR_OPERATOR_REVIEW,
            "dividend_event_review_status": READY_FOR_OPERATOR_ASSESSMENT,
            "dividend_event_authority_status": candidate_service.NOT_CREATED,
            "dividend_event_freeze_status": candidate_service.NOT_FROZEN,
            ...
            "dividend_history_status": candidate_service.NOT_FETCHED,
            "dividend_event_count_status": candidate_service.NOT_EVALUATED,
        ...
        for field in (
            ...
            "source_split_event_candidate_review_digest",
            "per_ticker_dividend_event_candidate_digest",
            "per_ticker_dividend_event_review_digest",
        ):
        ...
        _expect(
            entry["per_ticker_dividend_event_candidate_digest"],
            candidate_service.per_ticker_dividend_event_candidate_digest_v1(
                _candidate_entry_from_review_entry(entry)
            ),
            f"{ticker}.per_ticker_dividend_event_candidate_digest",
        )
        _expect(
            entry["per_ticker_dividend_event_review_digest"],
            per_ticker_dividend_event_review_digest_v1(entry),
            f"{ticker}.per_ticker_dividend_event_review_digest",
        )


def validate_dividend_event_authority_candidate_review_package_v1(
    review_package: dict[str, Any],
) -> dict[str, Any]:
    """Validate the dividend-event authority candidate review package."""
    ...
    if review_package.get("dividend_event_authority_candidate_binding_mode") not in {
        DIVIDEND_EVENT_AUTHORITY_CANDIDATE_STATUS_BINDING,
        DIVIDEND_EVENT_AUTHORITY_CANDIDATE_OBJECT_BINDING,
    }:
        raise DividendEventAuthorityCandidateReviewPackageError(
            "dividend_event_authority_candidate_binding_mode mismatch"
        )
    for field in (
        "created_offline",
    ...
    for field in (
        ...
        "corporate_action_authority_created",
        "corporate_action_authority_artifact_created",
        "ready_for_dividend_provider_evidence_request_approval",
    ...
    for field, expected in {
        "predictive_usefulness": acquisition.PREDICTIVE_USEFULNESS_NOT_ACCEPTED,
        "profitability": acquisition.PROFITABILITY_NOT_ACCEPTED,
        "reviewed_dividend_event_authority_candidate_kind": candidate_service.ARTIFACT_KIND_DIVIDEND_EVENT_AUTHORITY_CANDIDATE,
        "reviewed_dividend_event_authority_candidate_status": candidate_service.DIVIDEND_EVENT_AUTHORITY_READY_FOR_OPERATOR_REVIEW,
        "reviewed_dividend_event_authority_candidate_digest": EXPECTED_REVIEWED_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_DIGEST,
        "reviewed_dividend_event_authority_candidate_checklist_total": EXPECTED_REVIEWED_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_CHECKLIST_TOTAL,
        "reviewed_dividend_event_authority_candidate_checklist_passed": EXPECTED_REVIEWED_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_CHECKLIST_PASSED,
        "reviewed_dividend_event_authority_candidate_checklist_failed": EXPECTED_REVIEWED_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_CHECKLIST_FAILED,
        "reviewed_dividend_event_authority_candidate_blocker_count": EXPECTED_REVIEWED_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_BLOCKER_COUNT,
        "dividend_event_authority_candidate_digest": EXPECTED_REVIEWED_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_DIGEST,
        ...
        "post_identity_freeze_registry_inventory_approval_digest": candidate_service.split_review.candidate_service.approval.review.plan.EXPECTED_POST_IDENTITY_FREEZE_REGISTRY_INVENTORY_APPROVAL_DIGEST,
        "post_identity_freeze_registry_inventory_candidate_review_package_digest": candidate_service.split_review.candidate_service.approval.review.plan.approval_service.EXPECTED_POST_IDENTITY_FREEZE_REGISTRY_INVENTORY_CANDIDATE_REVIEW_PACKAGE_DIGEST,
        "post_identity_freeze_registry_inventory_candidate_digest": candidate_service.split_review.candidate_service.approval.review.plan.approval_service.EXPECTED_POST_IDENTITY_FREEZE_REGISTRY_INVENTORY_CANDIDATE_DIGEST,
        "identity_authority_freeze_digest": candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.EXPECTED_IDENTITY_AUTHORITY_FREEZE_DIGEST,
        "identity_authority_candidate_review_package_digest": candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_IDENTITY_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_DIGEST,
        "identity_authority_candidate_digest": candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_IDENTITY_AUTHORITY_CANDIDATE_DIGEST,
        "live_ticker_validation_results_review_package_digest": candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_LIVE_TICKER_VALIDATION_RESULTS_REVIEW_PACKAGE_DIGEST,
        "live_ticker_validation_execution_digest": candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_LIVE_TICKER_VALIDATION_EXECUTION_DIGEST,
        "ticker_universe_selection_approval_digest": candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_TICKER_UNIVERSE_SELECTION_APPROVAL_DIGEST,
        "source_split_event_authority_candidate_review_package_kind": candidate_service.split_review.ARTIFACT_KIND_SPLIT_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE,
        "source_split_event_authority_candidate_review_package_status": candidate_service.split_review.SPLIT_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_READY,
        "split_event_authority_candidate_review_package_digest": candidate_service.EXPECTED_SPLIT_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_DIGEST,
        "split_event_authority_candidate_digest": candidate_service.EXPECTED_SPLIT_EVENT_AUTHORITY_CANDIDATE_DIGEST,
        "target_universe": VALIDATION_TARGET_UNIVERSE,
        "corporate_action_plan_universe": VALIDATION_TARGET_UNIVERSE,
        "target_universe_count": 12,
        "authority_scope": candidate_service.split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.IDENTITY_AUTHORITY_ONLY,
        "dividend_event_authority_candidate_objective": candidate_service.DIVIDEND_EVENT_AUTHORITY_CANDIDATE_OBJECTIVE,
        "dividend_event_authority_candidate_scope": candidate_service.DIVIDEND_EVENT_AUTHORITY_CANDIDATE_SCOPE,
        "dividend_event_authority_creation_status": candidate_service.DIVIDEND_EVENT_AUTHORITY_CREATION_STATUS,
        "dividend_event_authority_freeze_status": candidate_service.DIVIDEND_EVENT_AUTHORITY_FREEZE_STATUS,
        "dividend_event_evidence_requirements": DIVIDEND_EVENT_EVIDENCE_REQUIREMENTS,
        "dividend_event_evidence_requirement_policy": DIVIDEND_EVENT_EVIDENCE_REQUIREMENT_POLICY,
        "dividend_policy_reconciliation_requirements": DIVIDEND_POLICY_RECONCILIATION_REQUIREMENTS,
        "future_dividend_provider_request_policy": FUTURE_DIVIDEND_PROVIDER_REQUEST_POLICY,
        "future_dividend_authority_chain": FUTURE_DIVIDEND_AUTHORITY_CHAIN,
        ...
        if field in {
            ...
            raise DividendEventAuthorityCandidateReviewPackageError(f"{field} missing")
    ...
    digest = review_package.get("dividend_event_authority_candidate_review_package_digest")
    if not isinstance(digest, str) or len(digest) != 64:
        raise DividendEventAuthorityCandidateReviewPackageError(
            "dividend_event_authority_candidate_review_package_digest missing"
        )
    _expect(
        digest,
        dividend_event_authority_candidate_review_package_digest_v1(review_package),
    ...
    return {
        "status": "DIVIDEND_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_VALID",
        "artifact_kind": review_package["artifact_kind"],
        "review_status": review_package["review_status"],
        "dividend_event_authority_candidate_review_package_digest": digest,
        "reviewed_dividend_event_authority_candidate_digest": review_package[
            "reviewed_dividend_event_authority_candidate_digest"
        ],
        ...
        "per_ticker_dividend_event_review_entry_count": len(
...
def build_dividend_event_authority_candidate_review_markdown_v1(
    review_package: dict[str, Any],
) -> str:
    """Render a sanitized dividend-event authority candidate review status document."""
    validation = validate_dividend_event_authority_candidate_review_package_v1(
        review_package
    )
    summary = review_package["review_summary"]
    lines = [
        "# MarketFlow Dividend Event Authority Candidate Operator Review Package Status",
        "",
        "## Title",
        "- Dividend Event Authority Candidate Operator Review Package v1.",
        "",
        "## Reviewed Dividend Event Authority Candidate",
        f"- Candidate kind: `{review_package['reviewed_dividend_event_authority_candidate_kind']}`",
        f"- Candidate status: `{review_package['reviewed_dividend_event_authority_candidate_status']}`",
        f"- Candidate digest: `{review_package['reviewed_dividend_event_authority_candidate_digest']}`",
        f"- Review package digest: `{validation['dividend_event_authority_candidate_review_package_digest']}`",
        "",
        ...
        f"- Split event candidate review package digest: `{review_package['split_event_authority_candidate_review_package_digest']}`",
        f"- Split event candidate digest: `{review_package['split_event_authority_candidate_digest']}`",
        "",
        "## Target Universe",
        ...
        f"- dividend_event_authority_candidate_objective: `{review_package['dividend_event_authority_candidate_objective']}`",
        f"- dividend_event_authority_candidate_scope: `{review_package['dividend_event_authority_candidate_scope']}`",
        f"- dividend_event_authority_creation_status: `{review_package['dividend_event_authority_creation_status']}`",
        f"- dividend_event_authority_freeze_status: `{review_package['dividend_event_authority_freeze_status']}`",
        "",
        "## Per-Ticker Dividend Event Review",
    ]
    lines.extend(
        f"- `{entry['ticker']}`: `{entry['dividend_event_review_status']}`, candidate digest `{entry['per_ticker_dividend_event_candidate_digest']}`, review digest `{entry['per_ticker_dividend_event_review_digest']}`"
        for entry in review_package["per_ticker_dividend_event_review_entries"]
    )
    lines.extend(["", "## Dividend Event Evidence Requirements"])
    lines.extend(f"- `{item}`" for item in review_package["dividend_event_evidence_requirements"])
    lines.extend(["", "## Dividend Policy Reconciliation Requirements"])
    lines.extend(
        f"- `{item}`"
        for item in review_package["dividend_policy_reconciliation_requirements"]
    )
    lines.extend(["", "## Future Dividend Provider Request Policy"])
    lines.extend(
        f"- `{key}`: `{value}`"
        for key, value in review_package["future_dividend_provider_request_policy"].items()
    )
    lines.extend(["", "## Future Dividend Authority Chain"])
    lines.extend(
        f"{index}. {step}"
        for index, step in enumerate(
            review_package["future_dividend_authority_chain"], start=1
        )
    )
    ...
    lines.extend(
        [
            "",
            "## Dividend Review Boundary",
            f"- dividend_event_authority_candidate_created: `{review_package['dividend_event_authority_candidate_created']}`",
            f"- dividend_event_authority_review_created: `{review_package['dividend_event_authority_review_created']}`",
            ...
            f"- dividend_provider_evidence_executed: `{review_package['dividend_provider_evidence_executed']}`",
            f"- dividend_provider_evidence_results_created: `{review_package['dividend_provider_evidence_results_created']}`",
            "",
            ...
            f"- Ready for dividend provider evidence request approval: `{summary['ready_for_dividend_provider_evidence_request_approval']}`",
            f"- Ready for dividend event authority freeze: `{summary['ready_for_dividend_event_authority_freeze']}`",
            "",
            "## Guardrails",
            "- Created offline: `True`",
            "- No Massive.com / Polygon provider dividend evidence was fetched.",
            "- No provider evidence request approval, provider evidence execution, dividend authority, or dividend freeze was created.",
...
def write_dividend_event_authority_candidate_review_package_v1(
    output_dir: str | Path,
    *,
    candidate: dict[str, Any] | None = None,
    ...
    review_package = build_dividend_event_authority_candidate_review_package_v1(
        candidate=candidate
    )
    validation = validate_dividend_event_authority_candidate_review_package_v1(
        review_package
    ...
    output_name = filename or "dividend_event_authority_candidate_review_package_v1.json"
    if Path(output_name).name != output_name or not output_name.endswith(".json"):
        raise DividendEventAuthorityCandidateReviewPackageError(
            "dividend event authority candidate review filename must be a simple JSON filename"
        )
    path = directory / output_name
    if path.exists():
        raise DividendEventAuthorityCandidateReviewPackageError(
            "dividend event authority candidate review output already exists"
        )
