"""Offline dividend-event authority candidate for identity-approved expanded-universe tickers."""
...
SCHEMA_VERSION_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_V1 = (
    "dividend_event_authority_candidate_v1"
)
DIVIDEND_EVENT_AUTHORITY_READY_FOR_OPERATOR_REVIEW = (
    "DIVIDEND_EVENT_AUTHORITY_READY_FOR_OPERATOR_REVIEW"
)
DIVIDEND_EVENT_CANDIDATE_READY_FOR_OPERATOR_REVIEW = (
    "DIVIDEND_EVENT_CANDIDATE_READY_FOR_OPERATOR_REVIEW"
)
DIVIDEND_EVENT_AUTHORITY_CANDIDATE_OBJECTIVE = (
    "CREATE_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_FOR_IDENTITY_APPROVED_EXPANDED_UNIVERSE"
)
...
DIVIDEND_EVENT_EVIDENCE_REQUIREMENT_POLICY = {
...
FUTURE_DIVIDEND_AUTHORITY_CHAIN = [
    "Dividend event authority candidate operator review package.",
    "Dividend provider evidence request approval ceremony, if live provider access is required.",
    "Dividend provider evidence execution.",
    "Dividend event evidence/results review package.",
    "Dividend policy reconciliation review.",
    "Dividend event authority candidate update or discrepancy triage, if required.",
    "Dividend event authority freeze ceremony.",
]

FUTURE_CORPORATE_ACTION_READINESS_CHAIN = [
    "Split event authority evidence/review/freeze chain.",
...
PLANNED_OUTPUT_NAMES = [
    "dividend_event_authority_candidate_operator_review_package",
    "dividend_provider_evidence_request_approval_ceremony_if_required",
    "dividend_provider_evidence_execution",
...
REQUIRED_CHECK_IDS = [
    ...
    "split_event_candidate_review_digest_bound",
    ...
    "dividend_event_candidate_objective_defined",
    ...
    "dividend_event_evidence_requirements_defined",
    "dividend_policy_reconciliation_requirements_defined",
    "future_dividend_provider_request_policy_defined",
    "future_dividend_authority_chain_defined",
...
class DividendEventAuthorityCandidateError(ValueError):
    """Raised when the dividend-event authority candidate violates guardrails."""
...
def _expect(actual: Any, expected: Any, field_name: str) -> None:
    if actual != expected:
        raise DividendEventAuthorityCandidateError(f"{field_name} mismatch")


def _expect_true(actual: Any, field_name: str) -> None:
    if actual is not True:
        raise DividendEventAuthorityCandidateError(f"{field_name} must be true")


def _expect_false(actual: Any, field_name: str) -> None:
    if actual is not False:
        raise DividendEventAuthorityCandidateError(f"{field_name} must be false")
...
def _source_binding(
    split_candidate_review_artifact: dict[str, Any] | None,
) -> dict[str, Any]:
    if split_candidate_review_artifact is not None:
        try:
            validation = split_review.validate_split_event_authority_candidate_review_package_v1(
                split_candidate_review_artifact
            )
        except split_review.SplitEventAuthorityCandidateReviewPackageError as exc:
            raise DividendEventAuthorityCandidateError(
                f"source split candidate review invalid: {exc}"
            ) from exc
        _expect(
        ...
        return deepcopy(split_candidate_review_artifact)
    return {
        "artifact_kind": split_review.ARTIFACT_KIND_SPLIT_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE,
        "review_status": split_review.SPLIT_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_READY,
        "split_event_authority_candidate_review_package_digest": (
            EXPECTED_SPLIT_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_DIGEST
        ),
        ...
        "post_identity_freeze_registry_inventory_candidate_review_package_digest": (
            split_review.candidate_service.approval.review.plan.approval_service.EXPECTED_POST_IDENTITY_FREEZE_REGISTRY_INVENTORY_CANDIDATE_REVIEW_PACKAGE_DIGEST
        ),
        "post_identity_freeze_registry_inventory_candidate_digest": (
            split_review.candidate_service.approval.review.plan.approval_service.EXPECTED_POST_IDENTITY_FREEZE_REGISTRY_INVENTORY_CANDIDATE_DIGEST
        ),
        ...
        "identity_authority_candidate_review_package_digest": (
            split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_IDENTITY_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_DIGEST
        ),
        "identity_authority_candidate_digest": (
            split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_IDENTITY_AUTHORITY_CANDIDATE_DIGEST
        ),
        ...
        "live_ticker_validation_execution_digest": (
            split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_LIVE_TICKER_VALIDATION_EXECUTION_DIGEST
        ),
...
def _dividend_entry_digest_payload(entry: dict[str, Any]) -> dict[str, Any]:
    payload = deepcopy(entry)
    payload.pop("per_ticker_dividend_event_candidate_digest", None)
    return payload


def per_ticker_dividend_event_candidate_digest_v1(entry: dict[str, Any]) -> str:
    """Return the deterministic semantic digest for one per-ticker dividend candidate entry."""
    return semantic_digest(_dividend_entry_digest_payload(entry))
    ...
    for ticker in source["target_universe"]:
        entry = {
            ...
            "identity_authority_status": (
                split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.IDENTITY_FREEZE_STATUS_FROZEN
            ),
            "registry_inventory_status": (
                split_review.candidate_service.approval.review.plan.approval_service.APPROVED_FOR_FUTURE_CORPORATE_ACTION_PLANNING_ONLY
            ),
            "corporate_action_plan_status": (
                split_review.candidate_service.approval.APPROVED_FOR_FUTURE_SPLIT_AND_DIVIDEND_AUTHORITY_CANDIDATES_ONLY
            ),
            "dividend_event_candidate_status": DIVIDEND_EVENT_CANDIDATE_READY_FOR_OPERATOR_REVIEW,
            ...
            "dividend_history_status": NOT_FETCHED,
            "dividend_event_count_status": NOT_EVALUATED,
            ...
            "source_split_event_candidate_review_digest": source[
                "split_event_authority_candidate_review_package_digest"
            ],
        }
        entry["per_ticker_dividend_event_candidate_digest"] = (
            per_ticker_dividend_event_candidate_digest_v1(entry)
        )
...
def _checklist(candidate: dict[str, Any]) -> list[dict[str, Any]]:
    entries = candidate.get("per_ticker_dividend_event_candidate_entries")
    ...
    return [
        ...
        _check("split_event_candidate_review_digest_bound", EXPECTED_SPLIT_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_DIGEST, candidate.get("split_event_authority_candidate_review_package_digest")),
        _check("split_event_candidate_digest_bound", EXPECTED_SPLIT_EVENT_AUTHORITY_CANDIDATE_DIGEST, candidate.get("split_event_authority_candidate_digest")),
        _check("registry_inventory_approval_digest_bound", split_review.candidate_service.approval.review.plan.EXPECTED_POST_IDENTITY_FREEZE_REGISTRY_INVENTORY_APPROVAL_DIGEST, candidate.get("post_identity_freeze_registry_inventory_approval_digest")),
        _check("registry_inventory_review_digest_bound", split_review.candidate_service.approval.review.plan.approval_service.EXPECTED_POST_IDENTITY_FREEZE_REGISTRY_INVENTORY_CANDIDATE_REVIEW_PACKAGE_DIGEST, candidate.get("post_identity_freeze_registry_inventory_candidate_review_package_digest")),
        _check("registry_inventory_candidate_digest_bound", split_review.candidate_service.approval.review.plan.approval_service.EXPECTED_POST_IDENTITY_FREEZE_REGISTRY_INVENTORY_CANDIDATE_DIGEST, candidate.get("post_identity_freeze_registry_inventory_candidate_digest")),
        _check("identity_freeze_digest_bound", split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.EXPECTED_IDENTITY_AUTHORITY_FREEZE_DIGEST, candidate.get("identity_authority_freeze_digest")),
        _check("identity_candidate_review_digest_bound", split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_IDENTITY_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_DIGEST, candidate.get("identity_authority_candidate_review_package_digest")),
        _check("identity_candidate_digest_bound", split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_IDENTITY_AUTHORITY_CANDIDATE_DIGEST, candidate.get("identity_authority_candidate_digest")),
        _check("live_validation_results_review_digest_bound", split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_LIVE_TICKER_VALIDATION_RESULTS_REVIEW_PACKAGE_DIGEST, candidate.get("live_ticker_validation_results_review_package_digest")),
        _check("live_validation_execution_digest_bound", split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_LIVE_TICKER_VALIDATION_EXECUTION_DIGEST, candidate.get("live_ticker_validation_execution_digest")),
        _check("ticker_universe_selection_approval_digest_bound", split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_TICKER_UNIVERSE_SELECTION_APPROVAL_DIGEST, candidate.get("ticker_universe_selection_approval_digest")),
        ...
        _check("ready_for_dividend_event_authority_candidate_true", True, candidate.get("ready_for_dividend_event_authority_candidate")),
        _check("dividend_event_candidate_objective_defined", DIVIDEND_EVENT_AUTHORITY_CANDIDATE_OBJECTIVE, candidate.get("dividend_event_authority_candidate_objective")),
        _check("dividend_event_candidate_scope_candidate_only", DIVIDEND_EVENT_AUTHORITY_CANDIDATE_SCOPE, candidate.get("dividend_event_authority_candidate_scope")),
        _check("dividend_event_authority_creation_status_not_created", DIVIDEND_EVENT_AUTHORITY_CREATION_STATUS, candidate.get("dividend_event_authority_creation_status")),
        _check("dividend_event_authority_freeze_status_not_frozen", DIVIDEND_EVENT_AUTHORITY_FREEZE_STATUS, candidate.get("dividend_event_authority_freeze_status")),
        _check("per_ticker_dividend_event_candidate_entries_12", 12, len(entries)),
        _check("per_ticker_dividend_event_candidate_status_ready", True, _all_entry_field(entries, "dividend_event_candidate_status", DIVIDEND_EVENT_CANDIDATE_READY_FOR_OPERATOR_REVIEW)),
        _check("per_ticker_dividend_event_authority_not_created", True, _all_entry_field(entries, "dividend_event_authority_status", NOT_CREATED)),
        _check("per_ticker_dividend_event_freeze_not_frozen", True, _all_entry_field(entries, "dividend_event_freeze_status", NOT_FROZEN)),
        ...
        _check("per_ticker_dividend_history_not_fetched", True, _all_entry_field(entries, "dividend_history_status", NOT_FETCHED)),
        _check("per_ticker_dividend_event_candidate_digests_present", True, _digests_present(entries, "per_ticker_dividend_event_candidate_digest")),
        _check("dividend_event_evidence_requirements_defined", DIVIDEND_EVENT_EVIDENCE_REQUIREMENTS, candidate.get("dividend_event_evidence_requirements")),
        _check("dividend_policy_reconciliation_requirements_defined", DIVIDEND_POLICY_RECONCILIATION_REQUIREMENTS, candidate.get("dividend_policy_reconciliation_requirements")),
        _check("future_dividend_provider_request_policy_defined", FUTURE_DIVIDEND_PROVIDER_REQUEST_POLICY, candidate.get("future_dividend_provider_request_policy")),
        _check("future_dividend_authority_chain_defined", FUTURE_DIVIDEND_AUTHORITY_CHAIN, candidate.get("future_dividend_authority_chain")),
        ...
        _check("dividend_event_authority_review_created_false", False, candidate.get("dividend_event_authority_review_created")),
        ...
        _check("dividend_provider_evidence_request_authorized_false", False, candidate.get("dividend_provider_evidence_request_authorized")),
        _check("dividend_provider_evidence_executed_false", False, candidate.get("dividend_provider_evidence_executed")),
        _check("dividend_provider_evidence_results_created_false", False, candidate.get("dividend_provider_evidence_results_created")),
        ...
        _check("no_dividend_event_authority_freeze_created", False, candidate.get("dividend_event_authority_freeze_created")),
        _check("no_dividend_provider_evidence_request_approval_created", False, candidate.get("dividend_provider_evidence_request_approval_created")),
        _check("no_dividend_provider_evidence_execution_created", False, candidate.get("dividend_provider_evidence_execution_created")),
...
def _digest_payload(candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    payload.pop("dividend_event_authority_candidate_digest", None)
    return payload


def dividend_event_authority_candidate_digest_v1(candidate: dict[str, Any]) -> str:
    """Return the deterministic semantic digest for the dividend-event candidate."""
    return semantic_digest(_digest_payload(candidate))


def build_dividend_event_authority_candidate_v1(
    *,
    split_candidate_review_artifact: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Build the offline dividend-event authority candidate."""
    source = _source_binding(split_candidate_review_artifact)
    candidate: dict[str, Any] = {
        "artifact_kind": ARTIFACT_KIND_DIVIDEND_EVENT_AUTHORITY_CANDIDATE,
        "schema_version": SCHEMA_VERSION_DIVIDEND_EVENT_AUTHORITY_CANDIDATE_V1,
        "candidate_status": DIVIDEND_EVENT_AUTHORITY_READY_FOR_OPERATOR_REVIEW,
        ...
        "source_split_event_authority_candidate_review_package_kind": source[
            "artifact_kind"
        ],
        "source_split_event_authority_candidate_review_package_status": source[
            "review_status"
        ],
        "split_event_authority_candidate_review_package_digest": source[
            "split_event_authority_candidate_review_package_digest"
        ],
        "split_event_authority_candidate_digest": source[
            "split_event_authority_candidate_digest"
        ],
        ...
        "dividend_event_authority_candidate_objective": DIVIDEND_EVENT_AUTHORITY_CANDIDATE_OBJECTIVE,
        "dividend_event_authority_candidate_scope": DIVIDEND_EVENT_AUTHORITY_CANDIDATE_SCOPE,
        "dividend_event_authority_creation_status": DIVIDEND_EVENT_AUTHORITY_CREATION_STATUS,
        "dividend_event_authority_freeze_status": DIVIDEND_EVENT_AUTHORITY_FREEZE_STATUS,
        ...
        "future_dividend_authority_chain": list(FUTURE_DIVIDEND_AUTHORITY_CHAIN),
    ...
    candidate["per_ticker_dividend_event_candidate_entries"] = _per_ticker_entries(
        source
    )
    ...
    candidate["dividend_event_authority_candidate_digest"] = (
        dividend_event_authority_candidate_digest_v1(candidate)
    )
    validate_dividend_event_authority_candidate_v1(candidate)
    return candidate
...
def _reject_forbidden_values(mapping: dict[str, Any], *, path: str = "candidate") -> None:
    for key, value in mapping.items():
        ...
        if key in {
            ...
            raise DividendEventAuthorityCandidateError(f"{current_path} must be false")
...
def _validate_entries(candidate: dict[str, Any]) -> None:
    ...
    for entry in entries:
        ...
        for field, expected in {
            "identity_authority_status": split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.IDENTITY_FREEZE_STATUS_FROZEN,
            "registry_inventory_status": split_review.candidate_service.approval.review.plan.approval_service.APPROVED_FOR_FUTURE_CORPORATE_ACTION_PLANNING_ONLY,
            "corporate_action_plan_status": split_review.candidate_service.approval.APPROVED_FOR_FUTURE_SPLIT_AND_DIVIDEND_AUTHORITY_CANDIDATES_ONLY,
            "dividend_event_candidate_status": DIVIDEND_EVENT_CANDIDATE_READY_FOR_OPERATOR_REVIEW,
            "dividend_event_authority_status": NOT_CREATED,
            "dividend_event_freeze_status": NOT_FROZEN,
            "provider_evidence_request_status": NOT_AUTHORIZED,
            ...
            "dividend_history_status": NOT_FETCHED,
            "dividend_event_count_status": NOT_EVALUATED,
            "acquisition_precondition_status": BLOCKED_UNTIL_SPLIT_AND_DIVIDEND_AUTHORITY_FROZEN_OR_EXPLICIT_POLICY,
            "runtime_use": _not_authorized(),
            "strategy_use": _not_authorized(),
            "paper_trading": _not_authorized(),
            "broker_execution": _not_authorized(),
        ...
        for field in (
            ...
            if not isinstance(digest, str) or len(digest) != 64:
                raise DividendEventAuthorityCandidateError(f"{field} missing")
        _expect(
        ...
        _expect(
            entry["source_split_event_candidate_review_digest"],
            candidate["split_event_authority_candidate_review_package_digest"],
            f"{ticker}.source_split_event_candidate_review_digest",
        )
        _expect(
            entry["per_ticker_dividend_event_candidate_digest"],
            per_ticker_dividend_event_candidate_digest_v1(entry),
...
def validate_dividend_event_authority_candidate_v1(
    candidate: dict[str, Any],
) -> dict[str, Any]:
    """Validate the dividend-event candidate while preserving all downstream guardrails."""
    if not isinstance(candidate, dict):
        raise DividendEventAuthorityCandidateError("candidate must be a JSON object")
    ...
    for field, expected in {
        "source_split_event_authority_candidate_review_package_kind": split_review.ARTIFACT_KIND_SPLIT_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE,
        "source_split_event_authority_candidate_review_package_status": split_review.SPLIT_EVENT_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_READY,
        ...
        "post_identity_freeze_registry_inventory_approval_digest": split_review.candidate_service.approval.review.plan.EXPECTED_POST_IDENTITY_FREEZE_REGISTRY_INVENTORY_APPROVAL_DIGEST,
        "post_identity_freeze_registry_inventory_candidate_review_package_digest": split_review.candidate_service.approval.review.plan.approval_service.EXPECTED_POST_IDENTITY_FREEZE_REGISTRY_INVENTORY_CANDIDATE_REVIEW_PACKAGE_DIGEST,
        "post_identity_freeze_registry_inventory_candidate_digest": split_review.candidate_service.approval.review.plan.approval_service.EXPECTED_POST_IDENTITY_FREEZE_REGISTRY_INVENTORY_CANDIDATE_DIGEST,
        "identity_authority_freeze_digest": split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.EXPECTED_IDENTITY_AUTHORITY_FREEZE_DIGEST,
        "identity_authority_candidate_review_package_digest": split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_IDENTITY_AUTHORITY_CANDIDATE_REVIEW_PACKAGE_DIGEST,
        "identity_authority_candidate_digest": split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_IDENTITY_AUTHORITY_CANDIDATE_DIGEST,
        "live_ticker_validation_results_review_package_digest": split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_LIVE_TICKER_VALIDATION_RESULTS_REVIEW_PACKAGE_DIGEST,
        "live_ticker_validation_execution_digest": split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_LIVE_TICKER_VALIDATION_EXECUTION_DIGEST,
        "ticker_universe_selection_approval_digest": split_review.candidate_service.approval.review.plan.approval_service.review_service.candidate_service.freeze_service.EXPECTED_TICKER_UNIVERSE_SELECTION_APPROVAL_DIGEST,
    ...
    if not _planned_outputs_not_generated(candidate):
        raise DividendEventAuthorityCandidateError("planned_outputs must not be generated")
    if not _planned_outputs_research_only(candidate):
        raise DividendEventAuthorityCandidateError("planned_outputs must be research only")
    ...
    digest = candidate.get("dividend_event_authority_candidate_digest")
    if not isinstance(digest, str) or len(digest) != 64:
        raise DividendEventAuthorityCandidateError(
            "dividend_event_authority_candidate_digest missing"
        )
    _expect(
        digest,
        dividend_event_authority_candidate_digest_v1(candidate),
    ...
    return {
        "status": "DIVIDEND_EVENT_AUTHORITY_CANDIDATE_VALID",
        "artifact_kind": candidate["artifact_kind"],
        "candidate_status": candidate["candidate_status"],
        "dividend_event_authority_candidate_digest": digest,
        ...
        "per_ticker_dividend_event_candidate_entry_count": len(
...
def build_dividend_event_authority_candidate_markdown_v1(
    candidate: dict[str, Any],
) -> str:
    """Render a sanitized dividend-event authority candidate status document."""
    validation = validate_dividend_event_authority_candidate_v1(candidate)
    summary = candidate["candidate_summary"]
    lines = [
        "# MarketFlow Dividend Event Authority Candidate Status",
        "",
        "## Title",
        "- Dividend Event Authority Candidate v1.",
        "",
        "## Purpose",
        "- Create an offline, digest-bound dividend event authority candidate for the identity-frozen expanded universe.",
        "- This candidate does not create dividend event authority or fetch provider dividend evidence.",
        "",
        ...
        f"- Split candidate review digest: `{candidate['split_event_authority_candidate_review_package_digest']}`",
        f"- Split candidate digest: `{candidate['split_event_authority_candidate_digest']}`",
        "",
        "## Target Universe",
        ...
        f"- dividend_event_authority_candidate_objective: `{candidate['dividend_event_authority_candidate_objective']}`",
        f"- dividend_event_authority_candidate_scope: `{candidate['dividend_event_authority_candidate_scope']}`",
        f"- dividend_event_authority_creation_status: `{candidate['dividend_event_authority_creation_status']}`",
        f"- dividend_event_authority_freeze_status: `{candidate['dividend_event_authority_freeze_status']}`",
        f"- Candidate digest: `{validation['dividend_event_authority_candidate_digest']}`",
        "",
        "## Per-Ticker Dividend Event Candidate Entries",
    ]
    lines.extend(
        f"- `{entry['ticker']}`: `{entry['dividend_event_candidate_status']}`, dividend authority `{entry['dividend_event_authority_status']}`, digest `{entry['per_ticker_dividend_event_candidate_digest']}`"
        for entry in candidate["per_ticker_dividend_event_candidate_entries"]
    )
    lines.extend(["", "## Dividend Event Evidence Requirements"])
    lines.extend(f"- `{item}`" for item in candidate["dividend_event_evidence_requirements"])
    lines.extend(["", "## Dividend Policy Reconciliation Requirements"])
    lines.extend(f"- `{item}`" for item in candidate["dividend_policy_reconciliation_requirements"])
    lines.extend(["", "## Future Dividend Provider Request Policy"])
    lines.extend(
        f"- `{key}`: `{value}`"
        for key, value in candidate["future_dividend_provider_request_policy"].items()
    )
    lines.extend(["", "## Future Dividend Authority Chain"])
    lines.extend(
        f"{index}. {step}"
        for index, step in enumerate(candidate["future_dividend_authority_chain"], start=1)
    )
    ...
    lines.extend(
        [
            ...
            f"- dividend_event_authority_review_created: `{candidate['dividend_event_authority_review_created']}`",
            ...
            f"- dividend_provider_evidence_request_authorized: `{candidate['dividend_provider_evidence_request_authorized']}`",
            f"- dividend_provider_evidence_executed: `{candidate['dividend_provider_evidence_executed']}`",
            f"- dividend_provider_evidence_results_created: `{candidate['dividend_provider_evidence_results_created']}`",
            "",
            "## Split Boundary",
            ...
            "- No dividend event authority, freeze, or provider evidence request authorization was created.",
            "- Split authority, corporate-action authority, acquisition, and dataset generation remain unauthorized.",
...
def write_dividend_event_authority_candidate_v1(
    output_dir: str | Path,
    *,
    split_candidate_review_artifact: dict[str, Any] | None = None,
    filename: str | None = None,
) -> dict[str, Any]:
    """Write the dividend-event authority candidate JSON artifact without overwriting output."""
    candidate = build_dividend_event_authority_candidate_v1(
        split_candidate_review_artifact=split_candidate_review_artifact
    )
    validation = validate_dividend_event_authority_candidate_v1(candidate)
    directory = Path(output_dir)
    directory.mkdir(parents=True, exist_ok=True)
    output_name = filename or "dividend_event_authority_candidate_v1.json"
    if Path(output_name).name != output_name or not output_name.endswith(".json"):
        raise DividendEventAuthorityCandidateError(
            "dividend event authority candidate filename must be a simple JSON filename"
        )
    path = directory / output_name
    if path.exists():
        raise DividendEventAuthorityCandidateError(
            "dividend event authority candidate output already exists"
        )
