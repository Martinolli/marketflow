"""Read-only split provider evidence execution for the expanded universe."""
...
from marketflow.services import split_event_provider_adapter_service as provider_adapter
from marketflow.services import split_provider_evidence_request_approval_service as approval


ARTIFACT_KIND_SPLIT_PROVIDER_EVIDENCE_EXECUTED = "SPLIT_EVENT_PROVIDER_EVIDENCE_EXECUTED"
ARTIFACT_KIND_SPLIT_PROVIDER_EVIDENCE_BLOCKED = "SPLIT_EVENT_PROVIDER_EVIDENCE_BLOCKED"
SCHEMA_VERSION_SPLIT_PROVIDER_EVIDENCE_EXECUTED_V1 = "split_provider_evidence_executed_v1"
SPLIT_PROVIDER_EVIDENCE_EXECUTED_READ_ONLY = "SPLIT_EVENT_PROVIDER_EVIDENCE_EXECUTED_READ_ONLY"
SPLIT_PROVIDER_EVIDENCE_BLOCKED_LIVE_GATE_OR_API_KEY_MISSING = (
    "SPLIT_EVENT_PROVIDER_EVIDENCE_BLOCKED_LIVE_GATE_OR_API_KEY_MISSING"
)
SPLIT_PROVIDER_EVIDENCE_BLOCKED_ENDPOINT_NOT_SELECTED = (
    "SPLIT_EVENT_PROVIDER_EVIDENCE_BLOCKED_ENDPOINT_NOT_SELECTED"
)
MARKETFLOW_ENABLE_LIVE_SPLIT_PROVIDER_EVIDENCE = "MARKETFLOW_ENABLE_LIVE_SPLIT_PROVIDER_EVIDENCE"
READ_ONLY_SPLIT_EVENT_EVIDENCE_REQUESTS_ONLY = approval.SPLIT_PROVIDER_EVIDENCE_REQUEST_SCOPE
RESEARCH_ONLY_NON_ACTIONABLE = approval.RESEARCH_ONLY_NON_ACTIONABLE
NOT_AUTHORIZED = approval.split_review.candidate_service.NOT_AUTHORIZED
NOT_CREATED = approval.NOT_CREATED
NOT_FROZEN = approval.split_review.candidate_service.NOT_FROZEN
PASS = approval.PASS
FAIL = approval.FAIL
BLOCKER = approval.BLOCKER

SPLIT_EVIDENCE_COLLECTED_READ_ONLY = "SPLIT_EVIDENCE_COLLECTED_READ_ONLY"
NO_SPLIT_EVENTS_RETURNED_BY_PROVIDER = "NO_SPLIT_EVENTS_RETURNED_BY_PROVIDER"
SPLIT_EVIDENCE_PROVIDER_RESPONSE_UNAVAILABLE = "SPLIT_EVIDENCE_PROVIDER_RESPONSE_UNAVAILABLE"
SPLIT_EVIDENCE_NOT_EVALUATED_BY_SELECTED_ENDPOINT = (
    "SPLIT_EVIDENCE_NOT_EVALUATED_BY_SELECTED_ENDPOINT"
)

OUTPUT_ROOT = Path(".marketflow") / "split_event_provider_evidence" / "expanded_universe_v1"
OUTPUT_FILENAMES = [
    "split_provider_evidence_run_manifest.json",
    "split_provider_request_receipts_sanitized.json",
    "split_event_results_sanitized.json",
    "split_event_absence_inventory.json",
    "split_event_failure_reason_inventory.json",
    "operator_review_summary.json",
]
TARGET_UNIVERSE = list(approval.VALIDATION_TARGET_UNIVERSE)
EXPECTED_SPLIT_PROVIDER_EVIDENCE_REQUEST_APPROVAL_DIGEST = (
    "7c7e93149fe118985fc218852d79e86b31c9ee5bbd75ebacd1890a3862d573db"
)
SPLIT_HISTORY_START_DATE = "1900-01-01"
PLANNED_SPLIT_EVIDENCE_CHECKS = [
    "split_event_history",
    "split_ratio",
...
class SplitProviderEvidenceExecutionError(ValueError):
    """Raised when split provider evidence execution violates the read-only contract."""
...
def _date_from_timestamp(timestamp_utc: str) -> str:
    try:
        ...
        raise SplitProviderEvidenceExecutionError("run_timestamp_utc must be ISO-8601 UTC text") from exc
...
def _base_output_fields() -> dict[str, Any]:
    return {
        ...
        "evidence_scope": READ_ONLY_SPLIT_EVENT_EVIDENCE_REQUESTS_ONLY,
...
def split_provider_evidence_execution_digest_v1(artifact: dict[str, Any]) -> str:
    """Return the deterministic digest for an executed read-only split evidence artifact."""
    return semantic_digest(_digest_payload(artifact, "split_provider_evidence_execution_digest"))
...
def _sanitized_event(row: Mapping[str, Any], *, index: int, ticker: str) -> dict[str, Any]:
    event = {
        ...
        "ex_date": row.get("ex_date") or row.get("execution_date"),
        "declaration_date": row.get("declaration_date"),
        "record_date": row.get("record_date"),
        "payable_date": row.get("payable_date"),
        "split_from": row.get("split_from") or row.get("splitFrom") or row.get("from_factor"),
        "split_to": row.get("split_to") or row.get("splitTo") or row.get("to_factor"),
        "split_ratio": row.get("split_ratio") or row.get("splitRatio") or row.get("ratio"),
        "provider_event_id_if_available": row.get("id") or row.get("event_id"),
        "reverse_split_flag_if_available": row.get("reverse_split"),
        "raw_event_index": index,
    }
    event["sanitized_split_event_digest"] = semantic_digest(event)
    return event


def _result_digest_payload(result: dict[str, Any]) -> dict[str, Any]:
    clone = deepcopy(result)
    clone.pop("sanitized_split_evidence_digest", None)
    return clone
...
def _not_evaluated_fields(events: list[dict[str, Any]]) -> list[str]:
    ...
    for field in (
        ...
        "execution_date",
        "ex_date",
        "provider_event_id_if_available",
        "reverse_split_flag_if_available",
    ):
        if any(event.get(field) is None for event in events):
            missing.add(field)
    missing.update({"split_adjustment_implication", "split_adjusted_price_impact_policy"})
    return sorted(missing)


def _split_check_statuses(result: dict[str, Any]) -> list[dict[str, Any]]:
    if result["split_provider_evidence_status"] == SPLIT_EVIDENCE_PROVIDER_RESPONSE_UNAVAILABLE:
        status = SPLIT_EVIDENCE_PROVIDER_RESPONSE_UNAVAILABLE
        return [{"check": check, "status": status} for check in PLANNED_SPLIT_EVIDENCE_CHECKS]
    statuses: list[dict[str, Any]] = []
    for check in PLANNED_SPLIT_EVIDENCE_CHECKS:
        if check in result["not_evaluated_fields"]:
            observed = SPLIT_EVIDENCE_NOT_EVALUATED_BY_SELECTED_ENDPOINT
        elif check == "split_event_absence_policy_if_no_splits_returned" and result["split_event_count"] == 0:
            observed = NO_SPLIT_EVENTS_RETURNED_BY_PROVIDER
        else:
            observed = SPLIT_EVIDENCE_COLLECTED_READ_ONLY
...
def _ticker_result_from_raw(ticker: str, raw: Mapping[str, Any]) -> dict[str, Any]:
    ...
    evidence_status = (
        SPLIT_EVIDENCE_COLLECTED_READ_ONLY
        if events
        else NO_SPLIT_EVENTS_RETURNED_BY_PROVIDER
    )
    result: dict[str, Any] = {
        "ticker": ticker,
        "provider_request_status": "REQUEST_PERFORMED_READ_ONLY",
        "split_provider_evidence_status": evidence_status,
        "split_event_count": len(events),
        "split_history_status": evidence_status,
        "split_absence_policy_status": (
            "NO_SPLIT_EVENT_ABSENCE_POLICY_APPLIED"
            if events
            else NO_SPLIT_EVENTS_RETURNED_BY_PROVIDER
        ),
        "split_event_results": events,
    ...
    result["split_evidence_checks"] = _split_check_statuses(result)
    result["sanitized_split_evidence_digest"] = semantic_digest(_result_digest_payload(result))
    return result
...
def _unavailable_ticker_result(ticker: str, reason: str) -> dict[str, Any]:
    result: dict[str, Any] = {
        ...
        "split_provider_evidence_status": SPLIT_EVIDENCE_PROVIDER_RESPONSE_UNAVAILABLE,
        "split_event_count": 0,
        "split_history_status": SPLIT_EVIDENCE_PROVIDER_RESPONSE_UNAVAILABLE,
        "split_absence_policy_status": "NOT_APPLIED_PROVIDER_RESPONSE_UNAVAILABLE",
        "split_event_results": [],
        "not_evaluated_fields": list(PLANNED_SPLIT_EVIDENCE_CHECKS),
...
def _summary(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    no_split = [item for item in results if item["split_provider_evidence_status"] == NO_SPLIT_EVENTS_RETURNED_BY_PROVIDER]
    collected = [item for item in results if item["split_provider_evidence_status"] == SPLIT_EVIDENCE_COLLECTED_READ_ONLY]
    return {
        ...
        "split_evidence_collected_count": len(collected),
        "no_split_events_returned_count": len(no_split),
...
def _output_payloads(*, artifact_seed: dict[str, Any], results: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    absences = [
        {
            "ticker": item["ticker"],
            "split_absence_policy_status": item["split_absence_policy_status"],
            "split_event_count": item["split_event_count"],
        }
        for item in results
        if item["split_provider_evidence_status"] == NO_SPLIT_EVENTS_RETURNED_BY_PROVIDER
    ]
    failures = [
        {
            "ticker": item["ticker"],
    ...
    return {
        ...
        OUTPUT_FILENAMES[2]: base | {"per_ticker_split_evidence_results": deepcopy(results)},
        OUTPUT_FILENAMES[3]: base | {"split_event_absence_inventory": absences},
        OUTPUT_FILENAMES[4]: base | {"split_event_failure_reason_inventory": failures},
        OUTPUT_FILENAMES[5]: base | {
...
def _executed_artifact_seed(
    *,
    ...
    return {
        "artifact_kind": ARTIFACT_KIND_SPLIT_PROVIDER_EVIDENCE_EXECUTED,
        "schema_version": SCHEMA_VERSION_SPLIT_PROVIDER_EVIDENCE_EXECUTED_V1,
        "execution_status": SPLIT_PROVIDER_EVIDENCE_EXECUTED_READ_ONLY,
        "evidence_scope": READ_ONLY_SPLIT_EVENT_EVIDENCE_REQUESTS_ONLY,
        "output_label": RESEARCH_ONLY_NON_ACTIONABLE,
        "run_timestamp_utc": timestamp,
        "split_history_start_date": SPLIT_HISTORY_START_DATE,
        "split_history_end_date": end_date,
        "selected_provider": provider_adapter.PROVIDER_NAME,
        "selected_endpoint": provider_adapter.MASSIVE_SPLIT_EVENTS_ENDPOINT,
        "selected_endpoint_stability": provider_adapter.MASSIVE_SPLIT_EVENTS_ENDPOINT_STABILITY,
        ...
        "per_ticker_split_provider_evidence_results": results,
        "generated_output_count": len(OUTPUT_FILENAMES),
        "execution_summary": summary,
        "output_digest_manifest": [],
        "next_required_task": "SPLIT_EVENT_EVIDENCE_RESULTS_REVIEW_PACKAGE",
    }


def _blocked_artifact(*, timestamp: str, reason: str) -> dict[str, Any]:
    return {
        "artifact_kind": ARTIFACT_KIND_SPLIT_PROVIDER_EVIDENCE_BLOCKED,
        "schema_version": SCHEMA_VERSION_SPLIT_PROVIDER_EVIDENCE_EXECUTED_V1,
        "execution_status": SPLIT_PROVIDER_EVIDENCE_BLOCKED_LIVE_GATE_OR_API_KEY_MISSING,
        "evidence_scope": READ_ONLY_SPLIT_EVENT_EVIDENCE_REQUESTS_ONLY,
        "output_label": RESEARCH_ONLY_NON_ACTIONABLE,
        "run_timestamp_utc": timestamp,
        "blocked_reason": reason,
        "required_live_gate": MARKETFLOW_ENABLE_LIVE_SPLIT_PROVIDER_EVIDENCE,
        ...
        "split_provider_evidence_execution_digest": "NOT_CREATED",
        "split_provider_evidence_request_approval_digest": (
            EXPECTED_SPLIT_PROVIDER_EVIDENCE_REQUEST_APPROVAL_DIGEST
        ),
        "target_universe": list(TARGET_UNIVERSE),
        ...
        "execution_summary": {
            ...
            "split_evidence_collected_count": 0,
            "no_split_events_returned_count": 0,
...
def execute_split_provider_evidence_v1(
    *,
    ...
    """Execute the approved read-only split evidence request or return a blocked artifact."""
    timestamp = run_timestamp_utc or _utc_now()
    end_date = _date_from_timestamp(timestamp)
    selected_endpoint_mode = "INJECTED_TRANSPORT_READ_ONLY" if transport is not None else "LIVE_HTTP_TRANSPORT_READ_ONLY"
    if transport is None and os.environ.get(MARKETFLOW_ENABLE_LIVE_SPLIT_PROVIDER_EVIDENCE) != "1":
    ...
    for ticker in TARGET_UNIVERSE:
        try:
            raw = provider_adapter.fetch_massive_split_events_v1(
                ticker=ticker,
                start_date=SPLIT_HISTORY_START_DATE,
                end_date=end_date,
                api_key=resolved_api_key,
                transport=transport,
                request_timestamp_utc=timestamp,
            )
        except provider_adapter.SplitEventProviderAdapterError as exc:
    ...
    artifact["split_provider_evidence_execution_digest"] = split_provider_evidence_execution_digest_v1(artifact)
    validate_split_provider_evidence_executed_v1(artifact)
    return artifact
...
def _expect(actual: Any, expected: Any, field_name: str) -> None:
    if actual != expected:
        raise SplitProviderEvidenceExecutionError(f"{field_name} mismatch")


def _expect_true(actual: Any, field_name: str) -> None:
    if actual is not True:
        raise SplitProviderEvidenceExecutionError(f"{field_name} must be true")


def _expect_false(actual: Any, field_name: str) -> None:
    if actual is not False:
        raise SplitProviderEvidenceExecutionError(f"{field_name} must be false")


def _expect_digest(actual: Any, field_name: str) -> None:
    if not isinstance(actual, str) or len(actual) != 64:
        raise SplitProviderEvidenceExecutionError(f"{field_name} missing")
...
def _validate_outputs(artifact: dict[str, Any]) -> None:
    ...
    if not isinstance(manifest, list) or len(manifest) != len(OUTPUT_FILENAMES):
        raise SplitProviderEvidenceExecutionError("output_digest_manifest mismatch")
...
def _validate_per_ticker_results(artifact: dict[str, Any]) -> None:
    entries = artifact.get("per_ticker_split_provider_evidence_results")
    if not isinstance(entries, list) or len(entries) != len(TARGET_UNIVERSE):
        raise SplitProviderEvidenceExecutionError("per-ticker split evidence results mismatch")
    _expect([entry.get("ticker") for entry in entries], TARGET_UNIVERSE, "per-ticker tickers")
    allowed_statuses = {
        SPLIT_EVIDENCE_COLLECTED_READ_ONLY,
        NO_SPLIT_EVENTS_RETURNED_BY_PROVIDER,
        SPLIT_EVIDENCE_PROVIDER_RESPONSE_UNAVAILABLE,
        SPLIT_EVIDENCE_NOT_EVALUATED_BY_SELECTED_ENDPOINT,
    }
    for entry in entries:
        if entry.get("split_provider_evidence_status") not in allowed_statuses:
            raise SplitProviderEvidenceExecutionError("split_provider_evidence_status mismatch")
        ...
        _expect(entry.get("split_event_authority_status"), NOT_CREATED, "split_event_authority_status")
        _expect(entry.get("split_event_freeze_status"), NOT_FROZEN, "split_event_freeze_status")
        ...
        _expect_digest(entry.get("sanitized_split_evidence_digest"), "sanitized_split_evidence_digest")
...
def _execution_checklist(artifact: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        _check("artifact_kind_executed", ARTIFACT_KIND_SPLIT_PROVIDER_EVIDENCE_EXECUTED, artifact.get("artifact_kind")),
        _check("execution_status_read_only", SPLIT_PROVIDER_EVIDENCE_EXECUTED_READ_ONLY, artifact.get("execution_status")),
        _check("split_request_authorized_true", True, artifact.get("split_provider_evidence_request_authorized")),
        ...
        _check("split_evidence_executed_true", True, artifact.get("split_provider_evidence_executed")),
        _check("split_evidence_results_created_true", True, artifact.get("split_provider_evidence_results_created")),
        ...
        _check("generated_output_count_6", 6, artifact.get("generated_output_count")),
        _check("summary_generated_output_count_6", 6, summary.get("generated_output_count")),
        _check("split_authority_not_created", False, artifact.get("split_event_authority_created")),
        _check("split_authority_not_frozen", False, artifact.get("split_event_authority_frozen")),
        _check("dividend_provider_request_not_authorized", False, artifact.get("dividend_provider_evidence_request_authorized")),
...
def validate_split_provider_evidence_executed_v1(artifact: dict[str, Any]) -> dict[str, Any]:
    """Validate a performed split provider evidence execution artifact."""
    if not isinstance(artifact, dict):
        raise SplitProviderEvidenceExecutionError("artifact must be a JSON object")
    _expect(artifact.get("artifact_kind"), ARTIFACT_KIND_SPLIT_PROVIDER_EVIDENCE_EXECUTED, "artifact_kind")
    _expect(artifact.get("schema_version"), SCHEMA_VERSION_SPLIT_PROVIDER_EVIDENCE_EXECUTED_V1, "schema_version")
    _expect(artifact.get("execution_status"), SPLIT_PROVIDER_EVIDENCE_EXECUTED_READ_ONLY, "execution_status")
    for field in (
    ...
    digest = artifact.get("split_provider_evidence_execution_digest")
    _expect_digest(digest, "split_provider_evidence_execution_digest")
    _expect(digest, split_provider_evidence_execution_digest_v1(artifact), "split_provider_evidence_execution_digest")
    checklist = _execution_checklist(artifact)
    failed = [item for item in checklist if item["status"] != PASS]
    if failed:
        raise SplitProviderEvidenceExecutionError(f"execution checklist failed: {failed[0]['check_id']}")
    summary = artifact["execution_summary"]
    return {
        "status": "SPLIT_PROVIDER_EVIDENCE_EXECUTED_VALID",
        "artifact_kind": artifact["artifact_kind"],
        "execution_status": artifact["execution_status"],
        "split_provider_evidence_execution_digest": digest,
        "split_provider_evidence_request_approval_digest": artifact["split_provider_evidence_request_approval_digest"],
        ...
        "split_evidence_collected_count": summary["split_evidence_collected_count"],
        "no_split_events_returned_count": summary["no_split_events_returned_count"],
...
def build_split_provider_evidence_execution_status_markdown_v1(artifact: dict[str, Any]) -> str:
    """Render a sanitized status document for split provider evidence execution."""
    blocked = artifact.get("artifact_kind") == ARTIFACT_KIND_SPLIT_PROVIDER_EVIDENCE_BLOCKED
    validation = None if blocked else validate_split_provider_evidence_executed_v1(artifact)
    summary = artifact["execution_summary"]
    per_ticker = artifact.get("per_ticker_split_provider_evidence_results", [])
    lines = [
        ...
        f"- Execution digest: `{artifact.get('split_provider_evidence_execution_digest', 'NOT_CREATED')}`",
        f"- Evidence scope: `{artifact['evidence_scope']}`",
        f"- Output label: `{artifact['output_label']}`",
        "",
        "## Source Evidence",
        f"- Split provider evidence request approval digest: `{artifact['split_provider_evidence_request_approval_digest']}`",
        "",
        "## Endpoint And Mode",
        f"- Selected provider: `{artifact.get('selected_provider', provider_adapter.PROVIDER_NAME)}`",
        f"- Selected endpoint: `{artifact.get('selected_endpoint', provider_adapter.MASSIVE_SPLIT_EVENTS_ENDPOINT)}`",
        f"- Selected endpoint stability: `{artifact.get('selected_endpoint_stability', provider_adapter.MASSIVE_SPLIT_EVENTS_ENDPOINT_STABILITY)}`",
        f"- Selected endpoint mode: `{artifact.get('selected_endpoint_mode', 'BLOCKED_NOT_ENABLED')}`",
        "",
        "## Target Universe",
        ...
        f"- Split evidence collected count: `{summary['split_evidence_collected_count']}`",
        f"- No-split events returned count: `{summary['no_split_events_returned_count']}`",
        f"- Not-evaluated count: `{summary['not_evaluated_count']}`",
    ...
    if per_ticker:
        lines.extend(
            f"- `{item['ticker']}`: `{item['split_provider_evidence_status']}`, events `{item['split_event_count']}`, response digest `{item['provider_response_digest']}`"
    ...
    lines.extend(
        [
            ...
            f"- dividend_provider_evidence_request_authorized: `{artifact['dividend_provider_evidence_request_authorized']}`",
            f"- dividend_provider_evidence_executed: `{artifact['dividend_provider_evidence_executed']}`",
