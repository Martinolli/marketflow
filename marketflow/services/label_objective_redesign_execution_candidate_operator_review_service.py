"""Offline operator review of the label-redesign execution candidate."""
...
from marketflow.services import (
    label_objective_redesign_execution_candidate_service as candidate_service,
)
...
SCHEMA_VERSION_LABEL_OBJECTIVE_REDESIGN_EXECUTION_CANDIDATE_REVIEW_V1 = (
...
EXPECTED_EXECUTION_CANDIDATE_DIGEST = (
    "92171d443cb676425a73dbdf484040f55a19371c5c0713b70e5ea6f37742b63d"
)
...
CHECK_IDS = [
    ...
    "label_objective_redesign_execution_candidate_review_created_true",
    ...
    "planned_workstreams_reviewed",
    "planned_label_family_outputs_10_reviewed",
    "planned_execution_outputs_reviewed",
    ...
    "no_label_objective_redesign_execution_approval_created",
...
class LabelObjectiveRedesignExecutionCandidateReviewError(ValueError):
    """Raised when the review violates its review-only boundary."""


def _expect(actual: Any, expected: Any, field: str) -> None:
    if actual != expected:
        raise LabelObjectiveRedesignExecutionCandidateReviewError(
            f"{field} mismatch"
        )
...
def _source_candidate(candidate: dict | None) -> dict[str, Any]:
    source = (
        candidate_service.build_label_objective_redesign_execution_candidate_v1()
        if candidate is None
        else deepcopy(candidate)
    )
    try:
        validation = (
            candidate_service.validate_label_objective_redesign_execution_candidate_v1(
                source
            )
        )
    except candidate_service.LabelObjectiveRedesignExecutionCandidateError as exc:
        raise LabelObjectiveRedesignExecutionCandidateReviewError(
            "source execution candidate is invalid"
        ) from exc
    _expect(
        source.get("label_objective_redesign_execution_candidate_digest"),
        EXPECTED_EXECUTION_CANDIDATE_DIGEST,
        "source execution candidate digest",
    )
    _expect(
        source.get("candidate_status"),
        candidate_service.LABEL_OBJECTIVE_REDESIGN_EXECUTION_CANDIDATE_READY_FOR_OPERATOR_REVIEW,
        "source execution candidate status",
    )
    _expect(source.get("review_summary", {}).get("blocker_count"), 0, "source blocker_count")
    _expect(
        validation.get("label_objective_redesign_execution_candidate_digest"),
        EXPECTED_EXECUTION_CANDIDATE_DIGEST,
        "validated source execution candidate digest",
    )
    return source


def per_ticker_label_objective_redesign_execution_candidate_review_digest_v1(
    entry: dict[str, Any],
) -> str:
...
def _per_ticker_review_entries(source: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in source["per_ticker_entries"]:
        entry = {
            "ticker": row["ticker"],
            ...
            "label_objective_redesign_approval_status": row[
                "label_objective_redesign_approval_status"
            ],
            "label_objective_redesign_execution_candidate_status": row[
                "label_objective_redesign_execution_candidate_status"
            ],
            "label_objective_redesign_execution_candidate_review_status": (
            ...
            "source_label_objective_redesign_execution_candidate_digest": source[
                "label_objective_redesign_execution_candidate_digest"
            ],
            "per_ticker_label_objective_redesign_execution_candidate_digest": row[
...
def _base_review(source: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "artifact_kind": ARTIFACT_KIND_LABEL_OBJECTIVE_REDESIGN_EXECUTION_CANDIDATE_REVIEW_PACKAGE,
        "schema_version": SCHEMA_VERSION_LABEL_OBJECTIVE_REDESIGN_EXECUTION_CANDIDATE_REVIEW_V1,
        "review_status": LABEL_OBJECTIVE_REDESIGN_EXECUTION_CANDIDATE_REVIEW_PACKAGE_READY,
        ...
        "label_objective_redesign_execution_approval_created": False,
        ...
        "reviewed_label_objective_redesign_execution_candidate_kind": source[
            "artifact_kind"
        ],
        "reviewed_label_objective_redesign_execution_candidate_status": source[
            "candidate_status"
        ],
        "reviewed_label_objective_redesign_execution_candidate_digest": source[
            "label_objective_redesign_execution_candidate_digest"
        ],
        "reviewed_label_objective_redesign_execution_candidate_checklist_total": source_summary[
            "total_checks"
        ],
        "reviewed_label_objective_redesign_execution_candidate_checklist_passed": source_summary[
            "passed_checks"
        ],
        "reviewed_label_objective_redesign_execution_candidate_checklist_failed": source_summary[
            "failed_checks"
        ],
        "reviewed_label_objective_redesign_execution_candidate_blocker_count": source_summary[
            "blocker_count"
        ],
        **REQUIRED_DIGEST_FIELDS,
        ...
        "label_objective_redesign_execution_candidate_objective": source[
            "label_objective_redesign_execution_candidate_objective"
        ],
        "label_objective_redesign_execution_candidate_scope": source[
            "label_objective_redesign_execution_candidate_scope"
        ],
        "label_objective_redesign_execution_candidate_mode": source[
            "label_objective_redesign_execution_candidate_mode"
        ],
        "label_objective_redesign_execution_candidate_authority_status": source[
            "label_objective_redesign_execution_candidate_authority_status"
        ],
        "problem_basis": deepcopy(source["problem_basis"]),
        "reviewed_planned_execution_activities": deepcopy(
            source["planned_execution_activities"]
        ),
        "reviewed_planned_workstreams": deepcopy(source["planned_workstreams"]),
        "reviewed_planned_label_family_outputs": deepcopy(
            source["planned_label_family_outputs"]
        ),
        "reviewed_planned_execution_outputs": deepcopy(
            source["planned_execution_outputs"]
        ),
...
def _derived_checks(review: dict[str, Any]) -> dict[str, bool]:
    ...
    outputs = review.get("reviewed_planned_execution_outputs", [])
    counts = review.get("per_ticker_record_counts", {})
    return {
        "execution_candidate_kind_matches": review.get("reviewed_label_objective_redesign_execution_candidate_kind") == candidate_service.ARTIFACT_KIND_LABEL_OBJECTIVE_REDESIGN_EXECUTION_CANDIDATE,
        "execution_candidate_status_ready_for_review": review.get("reviewed_label_objective_redesign_execution_candidate_status") == candidate_service.LABEL_OBJECTIVE_REDESIGN_EXECUTION_CANDIDATE_READY_FOR_OPERATOR_REVIEW,
        "execution_candidate_digest_matches_expected": review.get("reviewed_label_objective_redesign_execution_candidate_digest") == EXPECTED_EXECUTION_CANDIDATE_DIGEST,
        "execution_candidate_checklist_zero_blockers": review.get("reviewed_label_objective_redesign_execution_candidate_checklist_total") == 54 and review.get("reviewed_label_objective_redesign_execution_candidate_checklist_passed") == 54 and review.get("reviewed_label_objective_redesign_execution_candidate_checklist_failed") == 0 and review.get("reviewed_label_objective_redesign_execution_candidate_blocker_count") == 0,
        "execution_candidate_digest_bound": review.get("label_objective_redesign_execution_candidate_digest") == EXPECTED_EXECUTION_CANDIDATE_DIGEST,
        "approval_digest_bound": review.get("label_objective_redesign_approval_digest") == candidate_service.EXPECTED_APPROVAL_DIGEST,
        "candidate_review_digest_bound": review.get("label_objective_redesign_candidate_review_package_digest") == candidate_service.EXPECTED_CANDIDATE_REVIEW_PACKAGE_DIGEST,
        "candidate_digest_bound": review.get("label_objective_redesign_candidate_digest") == candidate_service.EXPECTED_CANDIDATE_DIGEST,
        "operator_method_path_selection_digest_bound": review.get("operator_method_path_selection_digest") == candidate_service.candidate_service.EXPECTED_OPERATOR_METHOD_PATH_SELECTION_DIGEST,
        "method_diagnostic_digest_bound": review.get("predictive_evidence_method_diagnostic_review_package_digest") == candidate_service.candidate_service.EXPECTED_METHOD_DIAGNOSTIC_REVIEW_DIGEST,
        "planning_tree_digest_bound": review.get("predictive_evidence_planning_tree_review_package_digest") == candidate_service.candidate_service.EXPECTED_PLANNING_TREE_REVIEW_DIGEST,
        "latest_readiness_digest_bound": review.get("latest_readiness_rerun_using_refined_evidence_digest") == candidate_service.candidate_service.EXPECTED_LATEST_READINESS_DIGEST,
        "research_registry_digest_bound": review.get("research_registry_approval_digest") == candidate_service.candidate_service.EXPECTED_RESEARCH_REGISTRY_APPROVAL_DIGEST,
        "records_digest_bound": review.get("records_digest") == candidate_service.candidate_service.EXPECTED_RECORDS_DIGEST,
        "target_universe_12_preserved": review.get("target_universe_count") == 12 and review.get("target_universe") == TARGET_UNIVERSE,
        "target_universe_matches_execution_candidate_universe": review.get("target_universe") == TARGET_UNIVERSE,
        "records_digest_preserved": review.get("records_digest") == candidate_service.candidate_service.EXPECTED_RECORDS_DIGEST,
        ...
        "label_objective_redesign_approved_true": review.get("label_objective_redesign_approved") is True,
        "ready_for_label_objective_redesign_execution_candidate_true": review.get("ready_for_label_objective_redesign_execution_candidate") is True,
        "label_objective_redesign_execution_candidate_created_true": review.get("label_objective_redesign_execution_candidate_created") is True,
        "label_objective_redesign_execution_candidate_ready_for_operator_review_true": review.get("label_objective_redesign_execution_candidate_ready_for_operator_review") is True,
        "label_objective_redesign_execution_candidate_review_created_true": review.get("label_objective_redesign_execution_candidate_review_created") is True,
        "label_objective_redesign_authorized_false": review.get("label_objective_redesign_authorized") is False,
        "label_objective_redesign_executed_false": review.get("label_objective_redesign_executed") is False,
        "redesigned_label_generation_authorized_false": review.get("redesigned_label_generation_authorized") is False,
        "redesigned_label_generation_performed_false": review.get("redesigned_label_generation_performed") is False,
        "planned_execution_activities_reviewed": [item.get("activity_id") for item in review.get("reviewed_planned_execution_activities", []) if isinstance(item, dict)] == candidate_service.PLANNED_EXECUTION_ACTIVITIES,
        "planned_workstreams_reviewed": [item.get("workstream_id") for item in review.get("reviewed_planned_workstreams", []) if isinstance(item, dict)] == candidate_service.PLANNED_WORKSTREAMS,
        "planned_label_family_outputs_10_reviewed": [item.get("label_family_candidate_id") for item in review.get("reviewed_planned_label_family_outputs", []) if isinstance(item, dict)] == candidate_service.candidate_service.LABEL_FAMILY_CANDIDATES,
        "planned_execution_outputs_reviewed": [item.get("output_id") for item in outputs if isinstance(item, dict)] == candidate_service.PLANNED_EXECUTION_OUTPUTS,
        "planned_outputs_not_generated": isinstance(outputs, list) and len(outputs) == 8 and all(item.get("output_status") == "PLANNED_NOT_GENERATED" for item in outputs if isinstance(item, dict)),
        "planned_outputs_research_only": isinstance(outputs, list) and len(outputs) == 8 and all(item.get("authority") == candidate_service.candidate_service.RESEARCH_ONLY_NON_ACTIONABLE for item in outputs if isinstance(item, dict)),
        ...
        "per_ticker_candidate_digests_present": isinstance(entries, list) and len(entries) == 12 and all(isinstance(item.get("per_ticker_label_objective_redesign_execution_candidate_digest"), str) and len(item["per_ticker_label_objective_redesign_execution_candidate_digest"]) == 64 for item in entries if isinstance(item, dict)),
        "per_ticker_review_digests_present": isinstance(entries, list) and len(entries) == 12 and all(isinstance(item.get("per_ticker_label_objective_redesign_execution_candidate_review_digest"), str) and len(item["per_ticker_label_objective_redesign_execution_candidate_review_digest"]) == 64 and item["per_ticker_label_objective_redesign_execution_candidate_review_digest"] == per_ticker_label_objective_redesign_execution_candidate_review_digest_v1(item) for item in entries if isinstance(item, dict)),
        ...
        "no_label_objective_redesign_execution_approval_created": review.get("label_objective_redesign_execution_approval_created") is False,
...
def label_objective_redesign_execution_candidate_review_package_digest_v1(
...
def build_label_objective_redesign_execution_candidate_review_package_v1(
    candidate: dict | None = None,
) -> dict:
    """Build a non-authorizing review of the exact execution candidate."""
    ...
    review["label_objective_redesign_execution_candidate_review_package_digest"] = (
        label_objective_redesign_execution_candidate_review_package_digest_v1(review)
    )
    validate_label_objective_redesign_execution_candidate_review_package_v1(review)
    return review
...
def _reject_forbidden_authority(value: Any, *, path: str = "review_package") -> None:
    forbidden_true_fields = {
        ...
        "label_objective_redesign_execution_approval_created",
...
def validate_label_objective_redesign_execution_candidate_review_package_v1(
    review_package: dict,
) -> dict:
    """Validate source bindings while keeping every execution gate closed."""
    ...
    if not isinstance(digest, str) or len(digest) != 64:
        raise LabelObjectiveRedesignExecutionCandidateReviewError(
            "execution candidate review package digest missing"
        )
    _expect(
        digest,
    ...
    return {
        "status": "LABEL_OBJECTIVE_REDESIGN_EXECUTION_CANDIDATE_REVIEW_PACKAGE_VALID",
        "artifact_kind": review_package["artifact_kind"],
        "review_status": review_package["review_status"],
        "label_objective_redesign_execution_candidate_review_package_digest": digest,
        "reviewed_label_objective_redesign_execution_candidate_digest": EXPECTED_EXECUTION_CANDIDATE_DIGEST,
...
def build_label_objective_redesign_execution_candidate_review_markdown_v1(
    review_package: dict,
) -> str:
    """Render a validated, non-authorizing review summary."""
    validate_label_objective_redesign_execution_candidate_review_package_v1(
        review_package
    )
    summary = review_package["review_summary"]
    lines = [
        "# MarketFlow Label Objective Redesign Execution Candidate Review Package",
        "",
        "## Title",
        ...
        f"- Kind/status: `{review_package['reviewed_label_objective_redesign_execution_candidate_kind']}` / `{review_package['reviewed_label_objective_redesign_execution_candidate_status']}`.",
        f"- Digest: `{review_package['reviewed_label_objective_redesign_execution_candidate_digest']}`.",
        "- Candidate checklist: `54 / 54` passed; `0` blockers.",
        "",
        "## Bound Evidence",
    ]
    lines.extend(f"- {field}: `{review_package[field]}`." for field in REQUIRED_DIGEST_FIELDS)
    lines.extend([
        "",
        ...
        f"- Objective: `{review_package['label_objective_redesign_execution_candidate_objective']}`.",
        "",
        "## Problem Basis",
    ])
    lines.extend(f"- {key}: `{value}`." for key, value in review_package["problem_basis"].items())
    for heading, key, id_key in [
        ("Reviewed Planned Execution Activities", "reviewed_planned_execution_activities", "activity_id"),
        ("Reviewed Planned Workstreams", "reviewed_planned_workstreams", "workstream_id"),
        ("Reviewed Planned Label Family Outputs", "reviewed_planned_label_family_outputs", "label_family_candidate_id"),
        ("Reviewed Planned Execution Outputs", "reviewed_planned_execution_outputs", "output_id"),
    ]:
        lines.extend(["", f"## {heading}"])
        lines.extend(f"- `{item[id_key]}`." for item in review_package[key])
    lines.extend(["", "## Per-Ticker Review Entries"])
    lines.extend(f"- `{item['ticker']}`: review ready; redesign execution unauthorized." for item in review_package["per_ticker_review_entries"])
    for heading, key in [
    ...
    lines.extend([
        ...
        "",
        "## Guardrails",
        "- Review only; no redesign authorization or execution, label or feature generation, predictive acceptance, runtime, trading, or recommendations.",
    ])
    return "\n".join(lines)


def write_label_objective_redesign_execution_candidate_review_package_v1(
    output_dir: str | Path,
    *,
    candidate: dict | None = None,
    ...
    review = build_label_objective_redesign_execution_candidate_review_package_v1(
        candidate
    )
    validation = (
        validate_label_objective_redesign_execution_candidate_review_package_v1(
    ...
    output_name = (
        filename
        or "label_objective_redesign_execution_candidate_review_package_v1.json"
    )
