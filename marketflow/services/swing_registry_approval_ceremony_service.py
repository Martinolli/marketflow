"""Offline SWING registry approval ceremony helpers."""
...
ARTIFACT_KIND_SWING_REGISTRY_APPROVED = "SWING_REGISTRY_APPROVED"
SCHEMA_VERSION_SWING_REGISTRY_APPROVAL_V1 = "swing_registry_approval_v1"
SWING_REGISTRY_APPROVED = "SWING_REGISTRY_APPROVED"
OPERATOR_DECISION_APPROVE_SWING_REGISTRY_ENTRY = "APPROVE_SWING_REGISTRY_ENTRY"
OPERATOR_ATTESTATION_VERSION_V1 = "swing_registry_approval_operator_attestation_v1"
REQUIRED_SWING_REGISTRY_APPROVAL_ATTESTATION_PHRASE = (
    "APPROVE SWING REGISTRY ENTRY AAPL SWING RTH_HALF_SESSION_195M "
    "2022-01-01 2025-12-31 RESEARCH_DATASET NOT_RUNTIME_AUTHORIZED"
)

EXPECTED_REGISTRY_REVIEW_PACKAGE_DIGEST = "ab433bb2c4b58cdd3a6ae287640877a1a8e443a631ebc479bf765f7a8d2b6f9e"
EXPECTED_REGISTRY_CANDIDATE_DIGEST = review.EXPECTED_REGISTRY_CANDIDATE_DIGEST
...
REMAINING_ROADMAP_AFTER_SWING_REGISTRY_APPROVAL = [
...
REQUIRED_APPROVAL_CHECK_IDS = [
    ...
    "swing_frozen_digest_matches_expected",
    ...
    "operator_swing_frozen_digest_confirmation_matches",
    ...
    "registry_eligibility_true",
    "registry_activation_true",
...
class SwingRegistryApprovalCeremonyError(ValueError):
    """Raised when a SWING registry approval ceremony violates guardrails."""
...
def _expect(actual: Any, expected: Any, field_name: str) -> None:
    if actual != expected:
        raise SwingRegistryApprovalCeremonyError(f"{field_name} mismatch")


def _expect_false(actual: Any, field_name: str) -> None:
    if actual is not False:
        raise SwingRegistryApprovalCeremonyError(f"{field_name} must be false")


def _expect_true(actual: Any, field_name: str) -> None:
    if actual is not True:
        raise SwingRegistryApprovalCeremonyError(f"{field_name} must be true")
...
def _source_review_package(registry_review_package: dict[str, Any] | None) -> dict[str, Any]:
    source_review = (
        ...
        else review.build_swing_registry_approval_candidate_review_package_v1()
    )
    try:
        validation = review.validate_swing_registry_approval_candidate_review_package_v1(source_review)
    except review.SwingRegistryOperatorReviewError as exc:
        raise SwingRegistryApprovalCeremonyError(f"source registry review package invalid: {exc}") from exc
...
def build_swing_registry_approval_attestation_v1(
    *,
    ...
    operator_decision: str = OPERATOR_DECISION_APPROVE_SWING_REGISTRY_ENTRY,
    operator_attestation_version: str = OPERATOR_ATTESTATION_VERSION_V1,
) -> dict[str, Any]:
    """Build a non-secret operator attestation for SWING registry approval."""
    return {
...
def _attestation_checks(attestation: dict[str, Any] | None) -> list[dict[str, Any]]:
    if not isinstance(attestation, dict):
        return [
            _check("operator_decision_approved", OPERATOR_DECISION_APPROVE_SWING_REGISTRY_ENTRY, None),
            _check("operator_attestation_phrase_matches", REQUIRED_SWING_REGISTRY_APPROVAL_ATTESTATION_PHRASE, None),
            ...
            _check("operator_swing_frozen_digest_confirmation_matches", registry.EXPECTED_SWING_FROZEN_DIGEST, None),
    ...
    return [
        _check("operator_decision_approved", OPERATOR_DECISION_APPROVE_SWING_REGISTRY_ENTRY, attestation.get("operator_decision")),
        _check("operator_attestation_phrase_matches", REQUIRED_SWING_REGISTRY_APPROVAL_ATTESTATION_PHRASE, attestation.get("operator_attestation_phrase")),
        ...
        _check("operator_swing_frozen_digest_confirmation_matches", registry.EXPECTED_SWING_FROZEN_DIGEST, attestation.get("operator_confirms_swing_frozen_digest")),
...
def _validated_operator_attestation(attestation: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(attestation, dict):
        raise SwingRegistryApprovalCeremonyError("operator_attestation must be a JSON object")
    for field in ("operator_reference", "operator_attestation_timestamp_utc", "operator_attestation_version"):
        value = attestation.get(field)
        if not isinstance(value, str) or not value.strip():
            raise SwingRegistryApprovalCeremonyError(f"{field} must be a non-empty string")
    failed = [item for item in _attestation_checks(attestation) if item["status"] != PASS]
    if failed:
        raise SwingRegistryApprovalCeremonyError(f"operator attestation failed: {failed[0]['check_id']}")
...
def _review_evidence(source_review: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "source_registry_review_package_digest": source_review["swing_registry_review_package_semantic_digest"],
        ...
        "swing_canonical_dataset_frozen": source_review["swing_canonical_dataset_frozen"],
        "swing_canonical_dataset_frozen_digest": source_review["swing_canonical_dataset_frozen_digest"],
        ...
        "swing_bar_count": source_review["swing_bar_count"],
        ...
        "cross_check_2025_01_swing_bars": source_review["cross_check_2025_01_swing_bars"],
...
def _approval_checklist(approved: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        _check("swing_canonical_dataset_frozen_true", True, approved.get("swing_canonical_dataset_frozen")),
        _check("swing_frozen_digest_matches_expected", registry.EXPECTED_SWING_FROZEN_DIGEST, approved.get("swing_canonical_dataset_frozen_digest")),
        ...
        _check("dataset_profile_swing", swing.DATASET_PROFILE_SWING, approved.get("dataset_profile")),
        _check("dataset_bar_rule_rth_half_session_195m", swing.DATASET_BAR_RULE_RTH_HALF_SESSION_195M, approved.get("dataset_bar_rule")),
        _check("swing_bar_count_1988", registry.EXPECTED_SWING_BAR_COUNT, approved.get("swing_bar_count")),
        ...
        *_attestation_checks(approved.get("operator_attestation") if isinstance(approved.get("operator_attestation"), dict) else None),
        _check("registry_approval_created_true", True, approved.get("registry_approval_created")),
        _check("registry_eligibility_true", True, approved.get("registry_eligibility")),
        _check("registry_activation_true", True, approved.get("registry_activation")),
...
def _summary(checklist: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "swing_registry_approval_authorized_by_operator": failed == 0,
...
def _digest_payload(approved_artifact: dict[str, Any]) -> dict[str, Any]:
    ...
    payload.pop("swing_registry_approved_semantic_digest", None)
    return payload


def swing_registry_approved_semantic_digest_v1(approved_artifact: dict[str, Any]) -> str:
    """Return the deterministic semantic digest for a SWING registry approval artifact."""
    return semantic_digest(_digest_payload(approved_artifact))


def build_swing_registry_approved_v1(
    *,
    registry_review_package: dict[str, Any] | None = None,
    operator_attestation: dict[str, Any],
) -> dict[str, Any]:
    """Build an offline SWING registry-approved artifact for the research dataset entry."""
    ...
    approved = {
        "artifact_kind": ARTIFACT_KIND_SWING_REGISTRY_APPROVED,
        "schema_version": SCHEMA_VERSION_SWING_REGISTRY_APPROVAL_V1,
        "approval_status": SWING_REGISTRY_APPROVED,
        ...
        "remaining_roadmap": list(REMAINING_ROADMAP_AFTER_SWING_REGISTRY_APPROVAL),
    ...
    approved["swing_registry_approved_semantic_digest"] = swing_registry_approved_semantic_digest_v1(approved)
    validate_swing_registry_approved_v1(approved)
    return approved


def validate_swing_registry_approved_v1(approved_artifact: dict[str, Any]) -> dict[str, Any]:
    """Validate SWING registry approval while preserving runtime and strategy guardrails."""
    if not isinstance(approved_artifact, dict):
        raise SwingRegistryApprovalCeremonyError("approved artifact must be a JSON object")
    _expect(approved_artifact.get("artifact_kind"), ARTIFACT_KIND_SWING_REGISTRY_APPROVED, "artifact_kind")
    _expect(approved_artifact.get("schema_version"), SCHEMA_VERSION_SWING_REGISTRY_APPROVAL_V1, "schema_version")
    _expect(approved_artifact.get("approval_status"), SWING_REGISTRY_APPROVED, "approval_status")
    for field in (
    ...
    for field, expected in {
        "source_registry_review_package_kind": review.ARTIFACT_KIND_SWING_REGISTRY_APPROVAL_CANDIDATE_REVIEW_PACKAGE,
        "source_registry_review_status": review.SWING_REGISTRY_APPROVAL_CANDIDATE_REVIEW_PACKAGE_READY,
        ...
        "source_registry_candidate_kind": registry.ARTIFACT_KIND_SWING_REGISTRY_APPROVAL_CANDIDATE,
        "source_registry_candidate_status": registry.SWING_REGISTRY_APPROVAL_READY_FOR_OPERATOR_REVIEW,
    ...
    if failed:
        raise SwingRegistryApprovalCeremonyError(f"approval checklist contains failed check: {failed[0]['check_id']}")
    ...
    _expect_true(
        summary.get("swing_registry_approval_authorized_by_operator"),
        "swing_registry_approval_authorized_by_operator",
    )
    ...
    digest = approved_artifact.get("swing_registry_approved_semantic_digest")
    if not isinstance(digest, str) or len(digest) != 64:
        raise SwingRegistryApprovalCeremonyError("swing_registry_approved_semantic_digest missing")
    _expect(digest, swing_registry_approved_semantic_digest_v1(approved_artifact), "swing_registry_approved_semantic_digest")
    return {
        "status": "SWING_REGISTRY_APPROVED_VALID",
        "artifact_kind": approved_artifact["artifact_kind"],
        "approval_status": approved_artifact["approval_status"],
        "swing_registry_approved_semantic_digest": digest,
        "registry_key": registry.PROPOSED_REGISTRY_KEY,
        "registry_scope": registry.PROPOSED_REGISTRY_SCOPE,
        "source_registry_review_package_digest": EXPECTED_REGISTRY_REVIEW_PACKAGE_DIGEST,
        "source_registry_candidate_digest": EXPECTED_REGISTRY_CANDIDATE_DIGEST,
...
def build_swing_registry_approved_markdown_v1(approved_artifact: dict[str, Any]) -> str:
    """Render a sanitized SWING registry approval status document."""
    validation = validate_swing_registry_approved_v1(approved_artifact)
    ...
    lines = [
        ...
        "",
        "## Title",
        "- SWING Registry Approval Ceremony v1.",
        "",
        ...
        f"- Registry approval created: `{approved_artifact['registry_approval_created']}`",
        f"- Registry eligibility: `{approved_artifact['registry_eligibility']}`",
        f"- Registry activation: `{approved_artifact['registry_activation']}`",
        f"- Approval digest: `{validation['swing_registry_approved_semantic_digest']}`",
        "",
        ...
        f"- SWING frozen digest: `{approved_artifact['swing_canonical_dataset_frozen_digest']}`",
        ...
        f"- SWING bar count: `{approved_artifact['swing_bar_count']}`",
        "",
        "## Registry Scope",
        ...
        f"- Registry approval authorized by operator: `{summary['swing_registry_approval_authorized_by_operator']}`",
...
def write_swing_registry_approved_v1(
    output_dir: str | Path,
    *,
    registry_review_package: dict[str, Any] | None = None,
    operator_attestation: dict[str, Any],
    ...
    """Write the SWING registry approval JSON artifact without overwriting output."""
    approved = build_swing_registry_approved_v1(
        registry_review_package=registry_review_package,
        operator_attestation=operator_attestation,
    )
    validation = validate_swing_registry_approved_v1(approved)
    directory = Path(output_dir)
    directory.mkdir(parents=True, exist_ok=True)
    output_name = filename or "AAPL_SWING_RTH_HALF_SESSION_195M_2022_2025_registry_approved_v1.json"
    if Path(output_name).name != output_name or not output_name.endswith(".json"):
        raise SwingRegistryApprovalCeremonyError("SWING registry approval filename must be a simple JSON filename")
    path = directory / output_name
    if path.exists():
        raise SwingRegistryApprovalCeremonyError("SWING registry approval output already exists")
