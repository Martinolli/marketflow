...
def save_timeframe_data(ticker: str, timeframe_analyses: dict) -> None:
    ...
    safe_ticker = sanitize_filename(ticker)  # <-- ensure filenames are safe on Windows
    ...
    for timeframe, data in timeframe_analyses.items():
        ...
        if price_data_valid and volume_data_valid:
            ...
            file_path = base_path / f"{safe_ticker}_{timeframe}.csv"
            ...
            file_path = base_path / f"{safe_ticker}_{timeframe}_price.csv"
            ...
            file_path = base_path / f"{safe_ticker}_{timeframe}_volume_{date_str}.csv"
